...
const PAGE_CONFIG = {
  ...
  "paper-method": {
    ...
    sections: [
      ...
      {
        title: "Prespecified estimand and confirmatory statistic",
        body: `<p>In controlled experiments, the inferential unit is an <strong>independently trained student replica</strong>, not an individual prompt or API sample. For replica <em>k</em>, candidate <em>i</em>, mechanism channel <em>r</em>, query <em>q</em>, and control stratum <em>c</em>, first average repeated generations within the cell and compute the signed contrast</p><div class="equation">d<sub>k,i,r,q,c</sub> = mean<sub>ℓ</sub> s<sub>r</sub>(S<sub>k,q,ℓ</sub>,T<sub>i,q,ℓ</sub>) − E<sub>C∈𝒞<sub>i,c</sub></sub>[mean<sub>ℓ</sub> s<sub>r</sub>(C<sub>q,ℓ</sub>,T<sub>i,q,ℓ</sub>)]. <span class="eq-label">(3)</span></div><p>Aggregate held-out queries with fixed stratum weights <em>w</em><sub>r,q</sub>, standardize each mechanism using control-only scale estimates, and combine the preregistered primary channels with fixed weights <em>λ</em><sub>r</sub>:</p><div class="equation">θ̂<sub>k,i,c</sub> = ∑<sub>r∈R<sub>primary</sub></sub> λ<sub>r</sub> ∑<sub>q∈Q<sub>r</sub></sub> w<sub>r,q</sub> d<sub>k,i,r,q,c</sub>/σ̂<sub>r,c</sub>, &nbsp; θ<sub>i,c</sub>=E<sub>k</sub>[θ̂<sub>k,i,c</sub>]. <span class="eq-label">(4)</span></div><p>The primary candidate estimand is the worst-control contrast <em>θ</em><sub>i</sub>=min<sub>c</sub> θ<sub>i,c</sub>. Detection uses an intersection–union test requiring evidence against every identification-critical control stratum. The initial primary composite uses response, reasoning, and preference channels with fixed preregistered weights after control normalization; safety and tool channels remain secondary. Missing primary channels trigger inconclusive rather than post-hoc reweighting.</p>`
      },
      {
        title: "Adaptive query selection",
        body: `<p>Split prompts and semantic templates into a <strong>probe-discovery set</strong> and a disjoint <strong>inference set</strong>, or use cross-fitting. Candidate disagreement and stability may select templates on the discovery split, but significance testing must use held-out instances or valid selective inference.</p><div class="equation">x* = arg max<sub>x∈𝒳<sub>discover</sub></sub> [ Disagree<sub>𝒯</sub>(x) · Stability(x) · (1 − Consensus<sub>𝒞</sub>(x)) ]. <span class="eq-label">(5)</span></div><p>Without this separation, adaptive prompt search overfits the same candidate outputs later used for testing. Compare against random, uncertainty-only, and semantic-diversity selection at equal discovery and inference budgets.</p>`
      },
      {
        title: "Candidate-wise decisions and evidence-supported sets",
        body: `<p>For every candidate, test positive evidence using dependence-aware resampling and family-wise-error or false-discovery control across candidates. Separately run a two-one-sided equivalence test against the preregistered negligible-effect margin <em>δ*</em>. Return the effect estimate, confidence interval, adjusted significance, achieved power, stability diagnostic, and decision:</p><div class="equation">d<sub>i</sub> ∈ { detected, not-detected, inconclusive }, &nbsp; Ê<sub>α</sub>(S) = {T<sub>i</sub> : d<sub>i</sub>=detected}. <span class="eq-label">(6)</span></div><p>The <strong>confirmatory hypothesis family</strong> contains one fixed-composite intersection–union test per candidate; candidates are corrected with Holm or a preregistered max-statistic procedure, while mechanism-specific analyses use a separate secondary FDR family. <strong>Detected:</strong> excess evidence clears the corrected positive threshold. <strong>Not detected:</strong> the entire equivalence interval falls inside <em>[−δ*,δ*]</em> and achieved power exceeds the preregistered target. <strong>Inconclusive:</strong> neither condition is met, including unstable nulls or insufficient queries. <em>Ê</em><sub>α</sub> is an evidence-supported candidate set, not a provenance set. Unique-candidate and teacher-set metrics remain secondary controlled-lineage evaluations [[2,7]].</p>`
      },
      {
        title: "Power-aware query budgeting",
        body: `<p>Discovery queries and inferential queries must be budgeted separately. Before final evaluation, estimate within-endpoint variance, control-to-control variance, prompt-cluster dependence, and repeat-sampling variance on a development split. Use these estimates to determine the number of held-out prompts and repeats needed to detect or exclude <em>δ*</em> at the declared error rates.</p><p>Report achieved power for every candidate and access tier, not only an aggregate budget curve. Adaptive querying may improve efficiency, but it cannot redefine <em>δ*</em> after observing test effects or silently turn underpowered cases into negative results.</p>`
      },
      {
        title: "Worked synthetic audit example",
        body: `<p>Consider four independently trained suspect replicas, three candidates, two identification-critical control strata, and a fixed primary composite over response, reasoning, and preference channels. The discovery split selects 90 templates; the confirmatory split contains 180 held-out queries (60 per channel), two generations per endpoint, equal channel weights, <em>δ*</em>=0.10, α=0.05, and target power 0.80. Query-level scores are averaged within repeats, aggregated with fixed weights, and reduced to the worst-control replicate-level effect.</p><table class="matrix"><thead><tr><th>Candidate</th><th>Same-base control effect</th><th>Alternative-teacher effect</th><th>Worst-control θ̂<sub>i</sub></th><th>Holm-adjusted positive test</th><th>Equivalence / power</th><th>Final decision</th></tr></thead><tbody><tr><td>T₁</td><td>0.44 [0.30, 0.58]</td><td>0.36 [0.22, 0.50]</td><td>0.36</td><td>p=0.006</td><td>CI outside ±0.10; power=0.96</td><td><strong>DETECTED</strong></td></tr><tr><td>T₂</td><td>0.04 [−0.03, 0.09]</td><td>0.02 [−0.05, 0.07]</td><td>0.02</td><td>p=0.74</td><td>Simultaneous CI inside ±0.10; power=0.91</td><td><strong>NOT DETECTED</strong></td></tr><tr><td>T₃</td><td>0.15 [−0.02, 0.32]</td><td>0.07 [−0.08, 0.22]</td><td>0.07</td><td>p=0.28</td><td>CI not equivalent; power=0.46</td><td><strong>INCONCLUSIVE</strong></td></tr></tbody></table><p>The evidence-supported candidate set is {T₁}. T₂ receives a genuine negative-evidence conclusion because the simultaneous interval is inside the preregistered negligible-effect region with adequate power. T₃ is not labeled negative: its interval is too wide. Mechanism-specific scores may explain why T₁ was detected, but they cannot change this confirmatory decision.</p><div class="notice">These values are pedagogical, not empirical results. Their purpose is to make every transformation—from query scores to a final label—auditable before code or experiments are frozen.</div>`
      },
      {
  ...
  "paper-benchmark": {
    ...
    sections: [
      ...
      },
      {
        ...
        body: `<p><strong>Primary estimand:</strong> replicate-level worst-control composite <em>θ</em><sub>i</sub> with simultaneous confidence intervals. <strong>Positive evidence:</strong> TPR at fixed FPR and family-wise error across the prespecified candidate family. <strong>Negative evidence:</strong> equivalence-test success at <em>δ*</em>, false equivalence on known positives, and achieved power <em>1−β</em>. <strong>Uncertainty:</strong> inconclusive rate decomposed into low power, unstable controls, missing primary channels, endpoint drift, and mechanism disagreement. <strong>Evidence-supported set:</strong> false inclusion, size, and controlled-lineage recall. <strong>Auxiliary tasks:</strong> closed-world top-k and mechanism-specific tests in a separate secondary FDR family. <strong>Efficiency:</strong> discovery and inference queries required for fixed error, equivalence, and power targets.</p><p>The confirmatory family is one fixed-composite intersection–union test per candidate, corrected across candidates with Holm or a preregistered max-statistic procedure. Mechanism-specific and control-stratum diagnostics are secondary and cannot upgrade the primary decision. Report sensitivity to <em>δ*</em>, hierarchical query-cluster intervals, and per-control-stratum performance.</p>`
      },
      {
      ...
      },
      {
        ...
        body: `<p>The following values are an illustrative minimum planning budget, not immutable model choices. Teacher and student identifiers must be frozen in a manifest before training. Costs are reported first as training runs, API calls, and token-equivalents so that volatile provider prices can be applied later.</p><table class="matrix"><thead><tr><th>Phase</th><th>Teacher / student design</th><th>Training workload</th><th>Query allocation</th><th>Illustrative audit cost</th></tr></thead><tbody><tr><td>P0 signal pilot</td><td>2 teacher families; 1 student-base family; response/reasoning/preference; 0.25× and 1.0× supervision doses; 3 replicas</td><td>36 positive students + 9 same-base/shared-data/common-third controls = <strong>45 runs</strong></td><td>90 discovery + 180 inference queries; 2 generations; 4-model blinded panel</td><td>≈26,460 generations; ≈15.9M token-equivalents at 600 tokens/call</td></tr><tr><td>P1 confirmatory controlled study</td><td>2 teacher families; 2 student bases; 3 mechanisms; 2 doses; 4 replicas</td><td>96 positive students + 32 identification controls = <strong>128 runs</strong></td><td>150 discovery + 300 inference queries; 2 generations; 5-model panel</td><td>≈119,700 generations; ≈71.8M token-equivalents</td></tr><tr><td>P4 simulated API stress</td><td>24 frozen suspects selected across effects and controls; 3 wrapper/decoder settings</td><td>No new training beyond selected checkpoints</td><td>300 held-out queries; 2 generations; 5-model panel</td><td>≈52,200 calls; ≈31.3M token-equivalents</td></tr><tr><td>P5 commercial pilot</td><td>3 suspect endpoints; 5 candidate/control endpoints; 3 audit dates</td><td>No training; observational only</td><td>120 discovery + 240 inference queries; 2 generations</td><td><strong>17,280 calls</strong>; ≈10.4M token-equivalents</td></tr><tr><td>Optional commercial confirmatory</td><td>5 suspect endpoints; 8 candidate/control endpoints; 3 dates</td><td>No training; run only after pilot GO</td><td>150 discovery + 300 inference queries; 3 generations</td><td><strong>52,650 calls</strong>; ≈31.6M token-equivalents</td></tr></tbody></table><div class="equation">API\ cost = ∑<sub>p</sub>(N<sub>in,p</sub>·rate<sub>in,p</sub> + N<sub>out,p</sub>·rate<sub>out,p</sub>). <span class="eq-label">(8)</span></div><p>Training manifests must also record measured GPU-hours, energy/runtime, teacher-generation tokens, and storage. The main run is gated on pilot variance estimates: replica counts, inference queries, and repeats are increased only when required to meet the frozen power target.</p>`
      },
      {
  ...
  "paper-roadmap": {
    ...
    sections: [
      ...
      },
      {
        ...
        body: `<h3>Primary candidate-evidence table</h3><table class="matrix"><thead><tr><th>Candidate</th><th>Control strata passed</th><th>θ̂<sub>i</sub></th><th>Simultaneous CI</th><th>Adjusted positive p</th><th>Equivalence result</th><th>Achieved power</th><th>Decision</th><th>Queries</th></tr></thead><tbody><tr><td>Tᵢ</td><td>same-base / alternative-teacher / shared-data / common-third</td><td>effect</td><td>[L,U]</td><td>p<sub>adj</sub></td><td>inside/outside ±δ*</td><td>1−β</td><td>detected / not detected / inconclusive</td><td>discovery + inference</td></tr></tbody></table><h3>Required companion tables</h3><ul><li><strong>Control-stratified effects:</strong> one row per candidate × control stratum, preserving easy and identification-critical negatives separately.</li><li><strong>Robustness:</strong> endpoint version/date, decoder, wrapper, repeated sampling, post-training transform, effect shift, and decision stability.</li><li><strong>Failures:</strong> candidate, failure category, low-power/null-instability/missing-channel cause, and whether the result changed to inconclusive.</li><li><strong>Commercial cases:</strong> preregistered candidates, observational effect, stability window, cost, and explicit claim boundary.</li></ul><h3>Bounded result language</h3><div class="claim-box"><b>Detected</b>“Under the preregistered candidate panel, matched controls, held-out protocol, and multiplicity correction, candidate-conditioned evidence for Tᵢ was detected (θ̂=…, simultaneous CI=…, p<sub>adj</sub>=…). This is not a causal teacher claim.”</div><div class="claim-box"><b>Not detected</b>“The simultaneous interval was contained within the preregistered negligible-effect region ±δ* with achieved power …; candidate-conditioned evidence for Tᵢ was not detected at that resolution.”</div><div class="claim-box"><b>Inconclusive</b>“Neither positive detection nor equivalence was established because …; no positive or negative lineage conclusion is supported.”</div><div class="claim-box"><b>Commercial API</b>“This endpoint-level result is observational and version-bounded; unlisted teachers, shared data, routing, and provider-side post-training remain unresolved.”</div><p>The limitations section must explicitly discuss candidate-pool incompleteness, control mismatch, training-data overlap, endpoint drift, hidden routing, mechanism-composite misspecification, query cost, and the inability of behavioral evidence alone to prove causality.</p>`
      },
      {
  ...
  home: {
    ...
    stats: [
      ["65", "curated papers, tools, disclosures, and reports"],
