...
const PAGE_CONFIG = {
  ...
  "paper-method": {
    ...
    title: "Candidate-Specific Boundary Transfer Testing",
    lead: "A black-box audit that searches for candidate-specific paired decision transitions and tests whether a suspect inherits those local boundaries.",
    callout: "Core intuition: distillation transfers a teacher's local decision geometry more reliably than it transfers surface wording. The primary test is a jointly calibrated max-statistic over response, reasoning, and preference transition channels; mechanism labels are multiplicity-controlled follow-ups.",
    sections: [
      {
        title: "Core intuition · local boundary transfer",
        body: `<p>Common correct answers are weak evidence because independently trained models share data, benchmarks, and broad capabilities. Distillation should instead preserve more of the teacher's <strong>local decision geometry</strong>: how a finite decision changes when one controlled feature of the input is changed. The audit therefore uses paired probes <em>b=(x⁰,x¹,a⁰,a¹)</em> where candidate <em>Tᵢ</em> is stable on both endpoints and exhibits the transition <em>a⁰→a¹</em>, while matched candidates or controls do not reproduce the same transition.</p><p>For mechanism channel <em>r</em>, estimate the candidate transition probability from repeated hard-label outputs and define a candidate-specific boundary margin:</p><div class="equation">Γ<sub>i,r</sub>(b)=P[T<sub>i</sub>(x⁰)=a⁰,T<sub>i</sub>(x¹)=a¹]−max<sub>C∈𝒞<sub>i</sub></sub>P[C(x⁰)=a⁰,C(x¹)=a¹]. <span class="eq-label">(2)</span></div><p>Discovery retains stable probes in a preregistered intermediate margin band: margins must be large enough to be candidate-specific but not so extreme that every capable model follows the same obvious transition. Rare-error agreement is retained as a secondary diagnostic; the primary object is paired transition transfer.</p>`
      },
      {
        title: "Concrete probe interfaces and data sources",
        body: `<table class="matrix"><thead><tr><th>Channel</th><th>Training source and dose</th><th>Audit interface</th><th>Primary per-query event</th></tr></thead><tbody><tr><td>Response / instruction</td><td>Prompts sampled from UltraChat 200k after contamination filtering; 5k and 20k teacher responses per student [[76]].</td><td>Four-option MCQ, constrained short answer, or JSON label</td><td>Candidate and suspect choose the same valid answer while matched controls do not.</td></tr><tr><td>Reasoning / process</td><td>Disjoint GSM8K- and MATH-derived templates with regenerated numbers; 5k and 20k structured teacher traces [[77,78]].</td><td>Choose the next valid subgoal, order four steps, or select the first erroneous step</td><td>Agreement on the candidate's decision boundary before the final answer.</td></tr><tr><td>Preference / judge</td><td>UltraFeedback prompts with two responses regenerated by fixed open models; 10k and 40k teacher A/B/tie judgments plus error categories [[79]].</td><td>A/B/tie and one of five critique labels</td><td>Pairwise preference agreement and margin stability under answer-order reversal.</td></tr><tr><td>Safety / filtering</td><td>Secondary extension using policy-matched benign/unsafe pairs</td><td>comply / refuse / conditional-compliance</td><td>Threshold and refusal-category agreement.</td></tr><tr><td>Tool policy</td><td>Secondary extension using fixed tool schemas</td><td>one tool name or no-tool</td><td>Tool-selection and argument-schema agreement.</td></tr></tbody></table><p>The primary family is frozen to response, reasoning, and preference. Safety and tool channels are reported separately until they pass their own identification controls. Discovery uses 3,000 candidate-only paired templates; the confirmatory split contains 1,200 disjoint pairs—400 per primary channel—with two independent generations per endpoint and unseen paraphrase/option-order seeds.</p>`
      },
      {
        title: "Channel-specific estimands and hierarchical omnibus",
        body: `<p>In controlled experiments, each independently trained student replica is an inferential unit. For suspect replica <em>k</em>, candidate <em>i</em>, channel <em>r</em>, and held-out paired probe <em>b</em>, average repeated generations within each endpoint and record the paired-transition event:</p><div class="equation">Y<sub>k,i,r,b</sub>=I[S<sub>k</sub>(x⁰)=a⁰,S<sub>k</sub>(x¹)=a¹]. <span class="eq-label">(3)</span></div><p>The channel effect is the suspect's transition rate minus the matched-control transition rate, estimated with query blocks nested within replicas:</p><div class="equation">θ<sub>i,r</sub>=E<sub>k,b</sub>[Y<sub>k,i,r,b</sub>]−E<sub>C∈𝒞<sub>i</sub>,b</sub>[Y<sub>C,i,r,b</sub>], &nbsp; Z<sub>i,r</sub>=θ̂<sub>i,r</sub>/SE(θ̂<sub>i,r</sub>). <span class="eq-label">(4)</span></div><p>Response, reasoning, and preference are calibrated separately, and <em>Z</em><sub>i,max</sub>=max<sub>r</sub>Z<sub>i,r</sub> is the primary candidate statistic. Crucially, calibration operates on the <strong>entire discovery-and-test algorithm</strong>, not on a candidate-selected probe set. In G0, teacher-arm labels are randomized only within the preregistered base × mechanism × dose blocks; for every randomization, candidate-specific discovery is rerun on cached discovery outputs, a new probe set is selected, and channel statistics are recomputed on the untouched inference split. The resulting maxima jointly include probe selection, channel selection, and candidate multiplicity, enabling maxT or step-down FWER control.</p><p>G1–G4 do not have randomized teacher assignment. Their statistics are compared with thresholds frozen from G0 plus a matched <strong>shadow-candidate library</strong>: every shadow candidate independently runs the same discovery algorithm before held-out scoring. Direct candidate/control permutation on an already selected probe set is forbidden. If the external case lacks enough matched shadow candidates or falls outside the G0 calibration envelope, its decision is inconclusive.</p>`
      },
      {
        title: "Selection-valid candidate-specific discovery",
        body: `<p>Prompt templates, task generators, and semantic families are split before any model query into a discovery partition and a disjoint inference partition. Candidate margin and stability are used only to select a transformation rule and task stratum on discovery data:</p><div class="equation">A<sub>i,r</sub>(D<sub>disc</sub>)=argmax<sub>A∈𝒜<sub>r</sub></sub>[Margin<sub>i</sub>(A)·Stability<sub>i</sub>(A)·(1−ControlTransfer(A))]. <span class="eq-label">(5)</span></div><p>The selected rule is instantiated with fresh entities, numbers, answer options, response pairs, and paraphrases from the inference partition; no discovered item or near duplicate is reused. Confirmatory uncertainty must rerun <em>A</em><sub>i,r</sub> inside every valid G0 randomization or bootstrap replicate. Cached discovery responses make this computational rerun query-free. Baselines—random pairs, uncertainty-only, semantic diversity, and raw disagreement—receive exactly the same discovery and inference budgets.</p>`
      },
      {
        title: "Candidate decision and mechanism follow-up",
        body: `<p>For each candidate, jointly test the three primary channels with the selection-valid algorithm-level max-statistic and control family-wise error across all candidates. Return the omnibus effect, adjusted p-value, channel effects and confidence intervals, stability diagnostics, and one of three decisions:</p><div class="equation">d<sub>i</sub>∈{detected,not\ detected,inconclusive}, &nbsp; Ê<sub>α</sub>(S)={T<sub>i</sub>:d<sub>i</sub>=detected}. <span class="eq-label">(6)</span></div><p><strong>Detected:</strong> the omnibus max-statistic exceeds its joint corrected threshold. <strong>Not detected:</strong> every primary channel's simultaneous interval is contained in its preregistered negligible-effect region and every channel meets the target power. <strong>Inconclusive:</strong> neither condition holds, including a strong but unstable channel, missing interface support, endpoint drift, or an underpowered channel. If detected, hierarchical follow-up tests identify which of response, reasoning, or preference is supported; failure of mechanism follow-up does not erase the candidate-level detection. The evidence-supported candidate set remains an observational summary, not a causal provenance set [[2,7]].</p>`
      },
      {
      ...
      },
      {
        ...
        body: `<p>Suppose three candidate teachers are tested with four independently trained suspect replicas and 1,200 held-out paired probes: 400 each for response, reasoning, and preference. Repeated generations are averaged within each probe endpoint; block bootstrap standard errors preserve replica and task-family dependence. The channel-wise negligible-effect margins are fixed at δ<sub>response</sub>=0.08, δ<sub>reasoning</sub>=0.08, and δ<sub>preference</sub>=0.06, with target power 0.80.</p><table class="matrix"><thead><tr><th>Candidate</th><th>Response θ̂ / Z</th><th>Reasoning θ̂ / Z</th><th>Preference θ̂ / Z</th><th>Joint maxT p</th><th>Follow-up / equivalence</th><th>Decision</th></tr></thead><tbody><tr><td>T₁</td><td>0.31 / 4.10</td><td>0.07 / 1.18</td><td>0.02 / 0.41</td><td>0.008</td><td>Response follow-up passes; others unsupported</td><td><strong>DETECTED · response-consistent</strong></td></tr><tr><td>T₂</td><td>0.01 / 0.22</td><td>−0.02 / −0.31</td><td>0.01 / 0.19</td><td>0.91</td><td>All simultaneous CIs inside their δ<sub>r</sub>; power ≥0.86</td><td><strong>NOT DETECTED</strong></td></tr><tr><td>T₃</td><td>0.04 / 0.71</td><td>0.14 / 2.03</td><td>−0.01 / −0.12</td><td>0.17</td><td>Reasoning CI crosses both zero and δ<sub>reasoning</sub>; power=0.54</td><td><strong>INCONCLUSIVE</strong></td></tr></tbody></table><p>The omnibus test detects T₁ because one channel contains strong candidate-specific boundary transfer; weak channels do not dilute it. The hierarchical follow-up supports response supervision but does not claim the exact hidden recipe. T₂ receives a negative-evidence decision only because all three channels are simultaneously equivalent with adequate power. T₃ remains inconclusive despite a suggestive reasoning effect.</p><div class="notice">The numbers are pedagogical, not empirical results. They demonstrate how joint channel selection, candidate multiplicity, mechanism follow-up, equivalence, and abstention interact before the analysis code is frozen.</div>`
      },
      {
      ...
      {
        title: "Novelty gate",
        body: `<p>Prior work already provides teacher tracing, behavioral provenance tests, active fingerprints, set-valued source inference, and finite-answer targeted counterfactual fingerprints [[1,2,7,14,15,80]]. The paper is methodologically novel only if <strong>paired candidate-specific boundary transfer</strong>, <strong>jointly calibrated unknown-mechanism max testing</strong>, and <strong>identification-valid controlled distillation</strong> improve at least one of three properties over these baselines: low-FWER sensitivity, mechanism recovery under orthogonal supervision, or query efficiency at fixed power.</p><div class="notice">The contribution is insufficient if raw answer agreement, rare-error overlap, random finite-answer probes, or a generic provenance score matches the proposed test on the shuffled-teacher, same-base, shared-data, and teacher-absent conditions.</div>`
      },
      {
        title: "Recommended method boundary",
        body: `<div class="survey-note"><b>Paper core</b>Candidate-only paired-probe discovery, held-out boundary-transition effects, jointly calibrated max-statistic detection, three-state decisions, and hierarchical channel follow-up.</div><div class="survey-note"><b>Causal validation</b>The G0 randomized controlled students test whether the statistic responds to the true teacher and the correct response/reasoning/preference mechanism.</div><div class="survey-note"><b>External validation</b>DeepSeek-R1 distill students, Llama 3.2, and Minitron are reported separately by disclosed procedure; commercial OpenRouter endpoints remain observational.</div><div class="survey-note"><b>Future extension</b>Multi-teacher contribution weights, direct-versus-indirect influence, unrestricted dependency graphs, and causal claims for private commercial systems.</div>`
      },
    ],
    view: "research-agenda",
  },
  "paper-benchmark": {
    ...
    sections: [
      ...
      },
      {
        ...
        body: `<p><strong>Primary estimands:</strong> channel-specific paired-transition effects <em>θ</em><sub>i,r</sub> and the jointly calibrated omnibus <em>Z</em><sub>i,max</sub>. <strong>Positive evidence:</strong> candidate-level TPR at fixed family-wise error under maxT/step-down correction. <strong>Negative evidence:</strong> simultaneous equivalence of every primary channel at its preregistered <em>δ</em><sub>r</sub>, false-equivalence rate on known positives, and achieved power for every channel. <strong>Mechanism recovery:</strong> conditional channel-wise precision/recall and confidence intervals only among omnibus-detected candidates. <strong>Uncertainty:</strong> inconclusive rate decomposed into low power, unstable probes, missing interfaces, endpoint drift, and conflicting channel evidence. <strong>Efficiency:</strong> candidate-only discovery calls and held-out paired inference calls required for fixed error and power targets.</p><p>Permutation blocks preserve task family, paired edits, replica, and control stratum. The same joint null calibrates channel selection and candidate multiplicity. Report channel effects separately, calibration by provenance stratum G0-G4, sensitivity to each <em>δ</em><sub>r</sub>, and performance on teacher-absent candidate pools.</p>`
      },
      {
      ...
      },
      {
        ...
        body: `<p>Costs are reported as teacher-generation tokens, training runs, local generations, and API calls; dollar costs are computed only after exact provider rates are frozen.</p><table class="matrix"><thead><tr><th>Phase</th><th>Workload</th><th>Training / model count</th><th>Audit allocation</th></tr></thead><tbody><tr><td>P0 controlled minimum</td><td>2 distillable teachers × 3 mechanisms × 2 token doses × 3 seeds on Qwen3-8B-Base</td><td><strong>36 positives + 36 factorial controls = 72 runs</strong>; up to 72M permission-compatible teacher-supervision tokens, with low-dose targets nested in the high-dose cache</td><td>3,000 candidate-only discovery pairs; frozen 1,200 held-out pairs (400/channel), 2 repeated generations</td></tr><tr><td>P0 external validation</td><td>R1-Distill-Qwen-7B and R1-Distill-Llama-8B plus their architecture-matched base checkpoints</td><td>No training</td><td>The same 1,200 held-out pair manifest; probe discovery may use R1 candidate outputs only</td></tr><tr><td>P1 cross-base scale-up</td><td>2 teachers × 3 mechanisms × 1 high dose × 3 seeds on Llama-3.1-8B</td><td><strong>18 additional positives + 18 factorial controls</strong></td><td>Reuse the frozen confirmatory families; no new test-item selection</td></tr><tr><td>P4 simulated serving stress</td><td>18 representative positives/controls × decoder, wrapper, rewrite, quantization, and post-training settings</td><td>No new core training</td><td>1,200 paired items per selected suspect; robustness claims restricted to tested paths</td></tr><tr><td>P5 OpenRouter observation</td><td>6 endpoints, 360 pairs, 2 temperatures, 3 dates</td><td>No training</td><td><strong>25,920 shared calls</strong>; candidate outputs cached by date</td></tr></tbody></table><div class="equation">API\ cost=∑<sub>p</sub>(N<sub>in,p</sub>·rate<sub>in,p</sub>+N<sub>out,p</sub>·rate<sub>out,p</sub>). <span class="eq-label">(8)</span></div><p>Before scale-up, the P0 campaign must control family-wise error on all controls, achieve the target power for both doses, and recover the correct supervision channel above the preregistered threshold. GPU-hours, teacher-output tokens, wall-clock time, checkpoints, and random seeds are recorded in a frozen run manifest.</p>`
      },
      {
  ...
  "paper-roadmap": {
    ...
    lead: "A scoped candidate-conditioned evidence-detection paper built on paired local-boundary transfer, selection-valid maxT inference, balanced controlled distillation, and observational commercial-API cases.",
    ...
    sections: [
      {
        ...
        body: `<div class="definition-box"><b>Thesis</b>Black-box distillation leaves candidate-specific traces in local decision boundaries: a student is more likely than matched non-teacher controls to preserve a candidate's stable response, reasoning, or preference transitions. These traces can be audited without knowing the supervision mechanism by candidate-only paired-probe discovery, selection-valid held-out inference, and a jointly calibrated channel-max test with detected / not-detected / inconclusive outputs.</div><p>A defensible title-style framing is <em>Candidate-Conditioned Black-box Distillation-Evidence Detection for Language Model APIs</em>. Titles implying verified provenance or “who taught” should be reserved for controlled-lineage experiments.</p>`
      },
      {
        title: "Contribution structure",
        body: `<ol><li><strong>Problem formulation.</strong> Candidate-conditioned black-box distillation-evidence detection, with three-state decisions and a claim ladder separating evidence from attribution and graph recovery.</li><li><strong>Signal.</strong> Candidate-specific paired decision-boundary transitions that suppress common-answer and surface-style confounds.</li><li><strong>Inference.</strong> Selection-valid algorithm-level randomization, a jointly calibrated max-statistic over unknown response/reasoning/preference mechanisms, and hierarchical channel follow-up.</li><li><strong>Evaluation.</strong> A balanced 72-run causal benchmark, stratified disclosed distillation families, hard negatives, simulated serving transformations, and terms-compliant OpenRouter observation.</li></ol>`
      },
      {
        title: "Suggested paper outline",
        body: `<div class="steps"><div class="step"><strong>Introduction.</strong> Commercial models expose outputs but hide training dependencies; common-answer similarity cannot establish teacher influence.</div><div class="step"><strong>Problem and threat model.</strong> Candidate-conditioned evidence, three-state decisions, candidate-pool incompleteness, and causal-versus-observational claim levels.</div><div class="step"><strong>Method.</strong> Paired boundary probes, candidate-only discovery, fresh inference instantiation, channel effects, nested selection-valid randomization, and maxT correction.</div><div class="step"><strong>Controlled benchmark.</strong> Balanced teacher/public/shuffled arms, orthogonal mechanisms, token-dose effects, teacher-absent pools, and hard negatives.</div><div class="step"><strong>External validation.</strong> DeepSeek-R1 distill, Llama 3.2, and Minitron reported separately by disclosed procedure.</div><div class="step"><strong>Commercial API cases.</strong> Directed OpenRouter candidate-evidence graphs with explicit version and causal limits.</div></div>`
      },
      {
        title: "Staged execution plan",
        body: `<table class="matrix"><thead><tr><th>Stage</th><th>Deliverable</th><th>Decision</th></tr></thead><tbody><tr><td>P0 · Probe feasibility</td><td>Construct paired transitions on the two distillable teachers; compare raw agreement, rare errors, random pairs, and active boundary probes</td><td>Continue only if boundary probes add held-out teacher-specific effect beyond all simple baselines</td></tr><tr><td>P0.5 · Freeze inference</td><td>Freeze task-family split, discovery algorithm, nested randomization, maxT family, δ<sub>r</sub>, power, exclusions, 1,200 inference pairs, and blinded run manifest</td><td>No confirmatory student label may be opened before the full algorithm is frozen</td></tr><tr><td>P1 · Balanced causal campaign</td><td>72 Qwen3-8B-Base runs across T₁/T₂/public/shuffled arms, three mechanisms, two token doses, and three seeds</td><td>Require FWER control, power at both doses, and correct mechanism follow-up</td></tr><tr><td>P2 · External disclosed pairs</td><td>R1 Qwen/Llama first; Llama 3.2 and Minitron only after P1 GO</td><td>Report separately by provenance stratum; no pooled causal accuracy</td></tr><tr><td>P3 · Cross-base and post-training</td><td>Balanced Llama-3.1-8B replication, full SFT, continued tuning, quantization, interpolation, rewriting, and leaked-probe removal</td><td>Retain only claims that survive explicitly tested paths</td></tr><tr><td>P4 · OpenRouter observation</td><td>Six version-pinned endpoints, three dates, directed ordered-pair analysis</td><td>Report candidate evidence and abstention, never hidden ground truth</td></tr></tbody></table>`
      },
      {
        title: "Go / no-go criteria",
        body: `<div class="property-grid"><div class="property-card"><b>GO</b>Candidate-wise tests control false positives, meet preregistered power/equivalence targets, and distinguish the true controlled teacher from every blinded matched control on contamination-screened held-out queries; inconclusive cases are calibrated and budgets are practical.</div><div class="property-card"><b>CONDITIONAL GO</b>Candidate-conditioned evidence detection works, but unique identification, teacher sets, or mechanism profiles are weak. Publish the narrower evidence-detection paper.</div><div class="property-card"><b>NO-GO</b>Performance is driven by family, benchmark accuracy, style, or public-data overlap; held-out effects vanish; the method requires student weights; or null calibration is unstable.</div><div class="property-card"><b>CLAIM DOWNGRADE</b>Commercial cases show candidate-specific consistency without causal ground truth. Report only observational evidence decisions.</div></div>`
      },
      {
        title: "Publication-ready reporting templates",
        body: `<h3>Primary candidate-evidence table</h3><table class="matrix"><thead><tr><th>Candidate</th><th>Response θ̂ / CI</th><th>Reasoning θ̂ / CI</th><th>Preference θ̂ / CI</th><th>Z<sub>max</sub></th><th>maxT p<sub>adj</sub></th><th>All-channel equivalence / minimum power</th><th>Decision</th><th>Supported follow-up</th></tr></thead><tbody><tr><td>Tᵢ</td><td>effect / [L,U]</td><td>effect / [L,U]</td><td>effect / [L,U]</td><td>statistic</td><td>p</td><td>yes/no; min(1−β<sub>r</sub>)</td><td>detected / not detected / inconclusive</td><td>response / reasoning / preference / none</td></tr></tbody></table><h3>Required companion tables</h3><ul><li><strong>G0 calibration:</strong> arm, mechanism, dose, seed, selected-probe count, channel effects, omnibus p, and whether the teacher and mechanism are recovered.</li><li><strong>Selection audit:</strong> discovery margin distribution, retained task families, randomization reruns, shadow-candidate count, and calibration-envelope diagnostics.</li><li><strong>External strata:</strong> G1–G4 reported separately; candidate snapshot match and disclosed procedure must be explicit.</li><li><strong>Robustness and failures:</strong> serving path, post-training transform, low-power/calibration-shift cause, and decision stability.</li></ul><h3>Bounded result language</h3><div class="claim-box"><b>Detected</b>“The selection-valid maxT test detected candidate-conditioned boundary-transfer evidence for Tᵢ (p<sub>adj</sub>=…). Hierarchical follow-up supports the … channel; this does not establish a complete or causal hidden training recipe.”</div><div class="claim-box"><b>Not detected</b>“All three simultaneous channel intervals were inside their preregistered negligible-effect regions with minimum achieved power …; evidence for Tᵢ was not detected at this resolution.”</div><div class="claim-box"><b>Inconclusive</b>“Neither the jointly corrected positive test nor all-channel equivalence was established because …; no positive or negative lineage conclusion is supported.”</div><div class="claim-box"><b>Commercial API</b>“This ordered endpoint-pair result is observational, version-bounded, and outside controlled random assignment; shared teachers, shared data, routing, and indirect influence remain unresolved.”</div><p>The limitations section must explicitly discuss candidate-pool incompleteness, selective-discovery calibration, shadow-candidate mismatch, training-data overlap, endpoint drift, hidden routing, mechanism misspecification, query cost, and the inability of behavioral evidence alone to prove causality.</p>`
      },
      {
  ...
  home: {
    ...
    stats: [
      ["85", "curated papers, tools, model cards, datasets, and disclosures"],
