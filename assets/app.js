...
const PAGE_CONFIG = {
  ...
  "paper-method": {
    ...
    sections: [
      ...
      },
      {
        ...
        body: `<div class="survey-note"><b>D0 population claim</b>Test whether ordinary teacher-output assignment causes an average candidate-specific behavioral echo across randomized student replicas. D0 does not validate a detector for one model.</div><div class="survey-note"><b>D1 detector claim</b>Only an independently calibrated and untouched confirmation cohort may support model-level sensitivity, specificity, false attribution, evidence-set precision, and abstention.</div><div class="survey-note"><b>Conditional direction</b>If only structural or action signatures transfer at the population level, retain those families and delete the boundary-centric claim before D1.</div><div class="survey-note"><b>NO-GO fallback</b>If no natural signature transfers under matched controls, stop passive lineage inference and move to non-causal convergence analysis, active trace watermarking, or provider-side telemetry [[88,89]].</div>`
      }
    ],
    view: "research-agenda",
  },
  "paper-benchmark": {
    ...
    lead: "Begin with public disclosed distillation pairs, then run a single-A100 four-arm feasibility study, and scale to multi-seed causal validation only after candidate-specific echoes survive matched controls.",
    callout: "Do not begin with the full 32-model D0 or 72-run mechanism campaign. The first objective is to falsify the signal cheaply: public checkpoints test external plausibility, a single-A100 pilot tests the pipeline, and independent student replicas establish the causal population effect.",
    sections: [
      {
        title: "Recommended execution plan for the behavioral-echo demo",
        body: `<p>The recommended demo is staged to minimize wasted training while preserving the logic of the final paper. It first asks whether known public distillation pairs show any candidate-specific directional echo, then tests the same signal under a controlled four-arm training design, and only then expands the number of independent students.</p><table class="matrix"><thead><tr><th>Gate</th><th>Concrete design</th><th>Compute</th><th>Decision and allowed conclusion</th></tr></thead><tbody><tr><td><strong>P0-public · zero-training screen</strong></td><td>Evaluate disclosed reasoning-distillation students and their base checkpoints, including the DeepSeek-R1-Distill family and OpenThoughts-derived artifacts. Discover probes from the candidate and matched controls only; test for diagonal candidate preference on fresh instances [[58,69]].</td><td>No student training; local inference on 7B-class checkpoints plus a fixed candidate endpoint where needed.</td><td>Continue only if public positives show larger base-normalized candidate echoes than alternative-teacher and base controls. This is external plausibility, not detector validation.</td></tr><tr><td><strong>P0-local · single-A100 pilot</strong></td><td>Use <code>Qwen/Qwen3-1.7B-Base</code> with four response-SFT LoRA-r16 arms: T₁ outputs, T₂ outputs, same-family shadow-teacher outputs, and public targets. Run one smoke seed, then four paired seed blocks if the pipeline is correct [[92,93,94,95]].</td><td>One A100 running models sequentially; finite-answer probes and short outputs. Smoke: 0.2–0.5M target tokens. Decisive pilot: up to 8M target tokens per student.</td><td>Require positive hardest-control residuals for both teachers, diagonal teacher selectivity, fresh-task replication, and collapse after candidate/probe shuffling. Four seeds support a direction decision only.</td></tr><tr><td><strong>D0-confirm · population validation</strong></td><td>Expand the same frozen four-arm protocol to eight paired seed blocks per arm, for 32 independently trained students. Do not alter probes, score definitions, exclusions, or the smallest effect of interest after labels are opened.</td><td>Single A100 is sufficient sequentially; multiple GPUs reduce wall-clock time but do not change the inferential unit.</td><td>Only this stage supports an average causal transfer claim. D1 individual-model detection begins only after D0 population GO.</td></tr></tbody></table><p>The primary demo statistic is the candidate transition rate of the teacher-trained student minus the hardest matched control. The initial boundary channel uses 240 held-out paired probes with budget curves at 20/40/80/120/240 pairs; structural behavior is retained as a secondary positive-control channel. Long free-form reasoning is deferred until the finite-answer signal is established.</p><div class="claim-box"><b>Practical stopping rule</b>If public pairs show no candidate selectivity, or the four-arm pilot is matched by shadow/public controls, stop the boundary-centered direction before committing to 32 or 72 training runs.</div>`
      },
      {
        title: "Compute and experimental precedent in related work",
        body: `<p>Prior work indicates that the proposed feasibility study is within single-A100 reach. The main cost is not fitting one model in memory; it is training enough independent student replicas to distinguish a teacher effect from seed variation.</p><table class="matrix"><thead><tr><th>Work</th><th>Reported hardware</th><th>Experimental scale</th><th>Design lesson for this project</th></tr></thead><tbody><tr><td><strong>Who Taught You That?</strong> [[1]]</td><td>2× NVIDIA A100; memory size not specified.</td><td>Five teacher candidates, GPT-2/OLMo-1B students, multiple summarization/QA/instruction tasks; 10k Alpaca prompts; lr 3e-5 and task-dependent batches.</td><td>Teacher attribution is feasible on modest hardware, but static POS/output aggregation must be a baseline rather than our claimed novelty.</td></tr><tr><td><strong>AgentEcho</strong> [[88]]</td><td>GPU type not disclosed; many main evaluations use provider APIs.</td><td>Controlled LoRA of Qwen2.5-14B-Instruct on 200 teacher trajectories, rank 16, lr 2e-5, 10 epochs.</td><td>A small trajectory set can move non-mandatory behavior; compare against an alternative teacher rather than only the pre-training checkpoint.</td></tr><tr><td><strong>Trace Rewriting</strong> [[89]]</td><td>Nodes with 4× A100 or H100; authors state one such GPU is sufficient for an individual experiment.</td><td>1B–8B students, vLLM generation, LoRA rank/alpha 128, batch 32, four epochs, outputs up to 1k–2k tokens.</td><td>One A100 can support our smaller finite-answer 1.7B/7B pipeline; long reasoning should remain a later secondary protocol.</td></tr><tr><td><strong>Counterfactual KD</strong> [[87]]</td><td>4× RTX A6000.</td><td>Qwen2.5-1.5B teacher to 0.5B student, paired counterfactual inputs, budgets from 8 to 512 examples, five independent runs.</td><td>Use controlled input pairs, query-budget curves, and repeated runs; do not infer a boundary from a single stochastic response.</td></tr><tr><td><strong>ESF</strong> [[96]]</td><td>2× NVIDIA H20 for fingerprint generation experiments.</td><td>0.5B–8B LLMs; fine-tuning, LoRA, backdoor, INT4/INT8, and replacement; ten independent derived models per tamper type.</td><td>Independent derived models—not thousands of prompts—are what make model-level error estimates credible.</td></tr><tr><td><strong>Membership and Memorization in KD</strong> [[90]]</td><td>Exact GPU configuration not reported.</td><td>Teacher/student families from GPT-2, OPT, and Llama; several KD methods and privacy attacks on Dolly-15k.</td><td>Report missing hardware honestly and separate mechanism grids from the core feasibility claim.</td></tr><tr><td><strong>Decision-only / boundary KD</strong> [[86,91]]</td><td>Exact GPU configuration not reported.</td><td>Multiple datasets and network pairs; repeated boundary searches or boundary-supporting samples.</td><td>These works motivate directional transitions, but their continuous image perturbations must be replaced by semantically controlled text edits.</td></tr></tbody></table><div class="survey-note"><b>Resource conclusion</b>A single A100 can complete the recommended public-checkpoint screen and the 1.7B four-arm feasibility study sequentially. Extra GPUs mainly shorten wall-clock time. The scientifically non-negotiable resource is the number of independently trained students.</div>`
      },
      {
      ...
      },
      {
        ...
        body: `<p>D0 asks one population-level causal question: under the frozen training pipeline, does assigning ordinary outputs from candidate teacher T₁ or T₂ cause a positive average behavioral-echo effect across independently trained students? D0 does <strong>not</strong> validate an individual-model detector.</p><table class="matrix"><thead><tr><th>Component</th><th>Frozen design</th></tr></thead><tbody><tr><td>Candidate teachers</td><td><code>Qwen/Qwen3-8B</code> and <code>meta-llama/Llama-3.1-8B-Instruct</code> [[92,93]]</td></tr><tr><td>Student</td><td><code>Qwen/Qwen3-1.7B-Base</code>, LoRA rank 16, shared initialization checkpoints across paired seed blocks [[94]]</td></tr><tr><td>Controls</td><td><code>Qwen/Qwen3-1.7B</code> shadow-teacher outputs and public-reference targets [[95]]</td></tr><tr><td>D0a high-dose arms</td><td>T₁, T₂, shadow, and public targets; 8 matched independent seed blocks per arm = <strong>32 trained models</strong></td></tr><tr><td>Training mixture</td><td>Identical prompts and <strong>8M target tokens</strong> per model; exact datasets and split construction are frozen separately before generation</td></tr><tr><td>Held-out measurement</td><td>300 boundary, 300 structural/POS, and 300 error/reasoning/action items; prompts are nested measurements</td></tr></tbody></table><p>The primary estimands are paired model-level contrasts τ<sub>T₁,s</sub> and τ<sub>T₂,s</sub>. Signature discovery rules are frozen from candidate-only development resources before arm labels are opened. The seed block—not the prompt—is randomized and resampled.</p><h3>D0a decision</h3><ul><li><strong>POPULATION GO:</strong> after family-wise correction, at least one signature family has a positive model-level effect for both T₁ and T₂, and each simultaneous lower confidence bound exceeds a preregistered smallest population effect of interest δ<sub>pop</sub>. The value of δ<sub>pop</sub> is frozen from blinded pilot/control variance and the intended query budget before arm labels are opened; the effect must repeat on fresh task families.</li><li><strong>CONDITIONAL GO:</strong> structural or error/action effects transfer for both teachers while boundary transfer does not. Continue without a boundary-centric thesis.</li><li><strong>NO-GO:</strong> no signature transfers for both teachers, effects vanish on fresh task families, or the apparent effect is equally explained by shadow/public arms. Stop passive auditing before developing an individual detector.</li></ul><p><strong>D0b dose study.</strong> Only after population GO, repeat the four arms at <strong>2M target tokens</strong>. Dose response requires a positive paired high-minus-low contrast for both teachers. D0a and D0b support population claims only.</p><p><strong>D0c decomposition.</strong> A later shuffled-output arm separates teacher style from input-conditioned transfer; it is not a null control.</p>`
      },
      {
        title: "D1 · Individual-model detector discovery, calibration, and confirmation",
        body: `<p>D1 begins only after D0 population GO. No D0 model enters D1 confirmation, and no D1 confirmation model is used to select features or thresholds. Seven source arms are fixed: T₁, T₂, public targets, and four alternative/shadow teachers matched on family, size, instruction tuning, data quality, capability, or common upstream influence.</p><table class="matrix"><thead><tr><th>Cohort</th><th>Model-level design</th><th>Permitted use</th></tr></thead><tbody><tr><td><strong>D1a discovery</strong></td><td>7 source arms × 4 new seed blocks = <strong>28 models</strong></td><td>Select and simplify the B/S/E feature definitions and remove unstable signatures. No deployed threshold is estimated.</td></tr><tr><td><strong>D1b calibration</strong></td><td>7 source arms × 8 new seed blocks = <strong>56 models</strong></td><td>Replay the entire frozen discovery pipeline inside model-level null resamples; freeze maxT calibration, candidate thresholds, practical-equivalence regions, and abstention.</td></tr><tr><td><strong>D1c untouched confirmation</strong></td><td>30 new T₁ models + 30 new T₂ models + 12 new models for each of five candidate-absent arms = <strong>120 models</strong></td><td>Estimate the frozen detector's operating characteristics. No feature, threshold, candidate-set, or exclusion change is allowed.</td></tr></tbody></table><p>Preregistered confirmation targets are evaluated with exact model-level confidence intervals: the one-sided 95% upper bound for candidate-absent false attribution must be below 5%; the one-sided 95% lower bound for sensitivity must be at least 80% for each candidate; wrong-candidate inclusion must have an upper bound below 5%; and abstention is reported separately for positive, negative, and out-of-envelope models. With 60 candidate-absent models, zero false attributions yields an upper bound just below 5%.</p><p>The primary result is the frozen operating point, not confirmation-set AUC optimization. AUC and score distributions are secondary diagnostics. Sample sizes may increase after a preregistered precision calculation but cannot decrease or be changed after confirmation begins.</p>`
      },
      {
      ...
      },
      {
        ...
        body: `<p>The trained student is always the independent experimental unit. Prompts, task templates, repeated generations, and decoding samples are nested measurements. D0 randomizes four arms within paired seed blocks and estimates population causal effects across those blocks.</p><p>D1 uses entirely new models. Discovery models may define features; calibration models may determine thresholds; confirmation models may only be scored by the frozen pipeline. Confirmation resampling and exact binomial intervals operate at the model level. No prompt-level bootstrap can replace missing trained models.</p><p>Commercial endpoints have no training replicas. Repeated dates and API sessions measure observational stability only and cannot inherit D0/D1 causal or detector-error guarantees.</p>`
      },
      {
      ...
      },
      {
        ...
        body: `<p>Costs are reported as teacher-output tokens, training runs, local generations, and API calls; dollar costs are computed only after exact provider rates are frozen.</p><table class="matrix"><thead><tr><th>Phase</th><th>Workload</th><th>Training / model count</th><th>Permitted conclusion</th></tr></thead><tbody><tr><td>D0a population feasibility</td><td>2 candidate teachers + 1 shadow teacher + public targets; 8 paired seeds; 8M target tokens</td><td><strong>32 trained models</strong></td><td>Average causal signature transfer only</td></tr><tr><td>D0b dose extension</td><td>Repeat four arms at 2M tokens after population GO</td><td><strong>32 additional models</strong>; D0 total 64</td><td>Population dose response only</td></tr><tr><td>D1a discovery</td><td>7 source arms × 4 new seeds</td><td><strong>28 models</strong></td><td>Feature-family development only</td></tr><tr><td>D1b calibration</td><td>7 source arms × 8 new seeds</td><td><strong>56 models</strong></td><td>Freeze maxT, thresholds, equivalence, and abstention</td></tr><tr><td>D1c confirmation</td><td>60 candidate-positive + 60 candidate-absent models</td><td><strong>120 untouched models</strong></td><td>Individual-model operating characteristics</td></tr><tr><td>G0 mechanism isolation</td><td>2 candidate teachers + public/shadow controls × 3 mechanisms × 2 doses × 3 seeds</td><td><strong>72 runs</strong></td><td>Mechanism-profile evidence only</td></tr><tr><td>External and OpenRouter</td><td>Disclosed models plus six endpoints across three dates</td><td>No training</td><td>External or observational evidence only</td></tr></tbody></table><p>D0 is the first scientific gate; D1 is the detector-reliability gate. G0 and commercial observation do not justify replacing either. Every run records GPU-hours, teacher-output tokens, wall-clock time, checkpoint hash, data manifest, model source arm, and random seed.</p>`
      },
      {
...
async function init() {
  try {
    const module = await import(new URL("content.mjs?v=20260723-benchmark-demo-v1", APP_ASSET_BASE).href);
    SITE_CONTENT = module.SITE_CONTENT || {};
    const methodModule = await import(new URL("paper-method-page.mjs?v=20260722-reader-first-v5", APP_ASSET_BASE).href);
...
const iaStylesheet = new URL("ia-style.css?v=20260723-ia-v17", APP_ASSET_BASE).href;
const iaRuntime = new URL("ia-runtime.js?v=20260723-ia-v17", APP_ASSET_BASE).href;
