export const SITE_CONTENT = {
  ...
  sectionRefs: {
    "paper-problem": [[1,2,7],[1,2,7,9,59],[1,2,3,6,7,9,14,15,16,24,25,26,27,59,64,65],[2,3,6],[],[36,38,39,44,47,49,56],[7,9,14,15,16,33],[45,47,49,59]],
  ...
  zhPages: {
    ...
    "paper-method": {
      ...
      sections: [
        ...
        { title: "预注册估计量与确认性统计量", body: `<p>受控实验中的推断单位是<strong>独立初始化、独立训练的学生 replica</strong>，而不是单个提示词或 API 样本。对 replica <em>k</em>、候选 <em>i</em>、机制通道 <em>r</em>、查询 <em>q</em> 和控制层 <em>c</em>，先在同一单元内平均重复生成，再计算有方向的对比：</p><div class="equation">d<sub>k,i,r,q,c</sub> = mean<sub>ℓ</sub> s<sub>r</sub>(S<sub>k,q,ℓ</sub>,T<sub>i,q,ℓ</sub>) − E<sub>C∈𝒞<sub>i,c</sub></sub>[mean<sub>ℓ</sub> s<sub>r</sub>(C<sub>q,ℓ</sub>,T<sub>i,q,ℓ</sub>)]. <span class="eq-label">(3)</span></div><p>使用固定查询权重 <em>w</em><sub>r,q</sub> 聚合留出查询，用只来自控制模型的尺度标准化机制通道，并用预注册固定权重 <em>λ</em><sub>r</sub> 合并主要通道：</p><div class="equation">θ̂<sub>k,i,c</sub> = ∑<sub>r∈R<sub>primary</sub></sub> λ<sub>r</sub> ∑<sub>q∈Q<sub>r</sub></sub> w<sub>r,q</sub> d<sub>k,i,r,q,c</sub>/σ̂<sub>r,c</sub>, &nbsp; θ<sub>i,c</sub>=E<sub>k</sub>[θ̂<sub>k,i,c</sub>]. <span class="eq-label">(4)</span></div><p>主要候选估计量取最不利控制层 <em>θ</em><sub>i</sub>=min<sub>c</sub> θ<sub>i,c</sub>，并采用交并检验，要求所有识别关键控制层均存在证据。第一版主要组合固定使用响应、推理和偏好通道，经控制标准化后按预注册权重聚合；安全与工具通道属于次要分析。缺失主要通道时返回结论不充分，不能事后重新加权。</p>` },
        { title: "自适应查询选择", body: `<p>将提示词和语义模板划分为<strong>探针发现集</strong>与互不重叠的<strong>推断集</strong>，或使用交叉拟合。发现集可以根据候选分歧和稳定性选择模板，但显著性检验必须使用留出实例或有效的选择后推断。</p><div class="equation">x* = arg max<sub>x∈𝒳<sub>discover</sub></sub> [ Disagree<sub>𝒯</sub>(x) · Stability(x) · (1 − Consensus<sub>𝒞</sub>(x)) ]. <span class="eq-label">(5)</span></div><p>否则自适应搜索会过拟合随后用于检验的同一批候选输出。应在相同发现与推断预算下，与随机、不确定性和语义多样性基线比较。</p>` },
        { title: "逐候选结论与证据支持集合", body: `<p>对每个候选使用依赖感知重采样检验正向证据，并在候选间控制 family-wise error 或 FDR；同时针对预注册可忽略效应边界 <em>δ*</em> 运行双单侧等效性检验。输出效应估计、置信区间、校正显著性、达到的检验功效、稳定性诊断和三态结论：</p><div class="equation">d<sub>i</sub> ∈ { 检测到, 未检测到, 结论不充分 }, &nbsp; Ê<sub>α</sub>(S) = {T<sub>i</sub> : d<sub>i</sub>=检测到}. <span class="eq-label">(6)</span></div><p><strong>确认性检验族</strong>包含每个候选一个固定组合的交并检验；跨候选使用 Holm 或预注册 max-statistic 程序校正，机制特异分析则进入独立的次要 FDR 检验族。<strong>检测到：</strong>超额证据超过校正后的正向阈值。<strong>未检测到：</strong>整个等效性区间落在 <em>[−δ*,δ*]</em> 内，且达到的功效超过预注册目标。<strong>结论不充分：</strong>两者均不成立，包括零分布不稳定或查询不足。<em>Ê</em><sub>α</sub> 是证据支持候选集合，不是来源集合；唯一候选与教师集合指标仅在受控谱系中辅助评价 [[2,7]]。</p>` },
        { title: "功效感知的查询预算", body: `<p>探针发现查询与正式推断查询必须分别预算。正式评价前，应在开发划分上估计端点内方差、控制模型间方差、提示簇相关性和重复采样方差，据此计算在声明的 α、β* 与 <em>δ*</em> 下需要多少留出提示词和重复采样。</p><p>每个候选和访问层都应报告实际达到的功效，而不只是总查询预算曲线。自适应查询可以提高效率，但不能在观察测试效应后重新定义 <em>δ*</em>，也不能把功效不足的结果写成负结论。</p>` },
        { title: "合成数据端到端审计示例", body: `<p>考虑 4 个独立训练的待审计模型 replica、3 个候选模型、2 个识别关键控制层，以及由响应、推理和偏好通道固定组合得到的主要统计量。发现集选择 90 个模板；确认性划分包含 180 个留出查询（每个通道 60 个）、每个端点重复生成 2 次、通道等权，设 <em>δ*</em>=0.10、α=0.05、目标功效为 0.80。逐查询得分先在重复采样内平均，再按固定权重聚合，最终取 replica 级最不利控制效应。</p><table class="matrix"><thead><tr><th>候选</th><th>同基础控制效应</th><th>替代教师控制效应</th><th>最不利控制 θ̂<sub>i</sub></th><th>Holm 校正正向检验</th><th>等效性 / 功效</th><th>最终结论</th></tr></thead><tbody><tr><td>T₁</td><td>0.44 [0.30, 0.58]</td><td>0.36 [0.22, 0.50]</td><td>0.36</td><td>p=0.006</td><td>区间超出 ±0.10；功效=0.96</td><td><strong>检测到</strong></td></tr><tr><td>T₂</td><td>0.04 [−0.03, 0.09]</td><td>0.02 [−0.05, 0.07]</td><td>0.02</td><td>p=0.74</td><td>同时区间位于 ±0.10 内；功效=0.91</td><td><strong>未检测到</strong></td></tr><tr><td>T₃</td><td>0.15 [−0.02, 0.32]</td><td>0.07 [−0.08, 0.22]</td><td>0.07</td><td>p=0.28</td><td>未满足等效性；功效=0.46</td><td><strong>结论不充分</strong></td></tr></tbody></table><p>证据支持候选集合为 {T₁}。T₂ 可以获得真正的负向证据结论，因为同时置信区间位于预注册可忽略效应区间内，并且功效充分。T₃ 不能被标记为未检测到，因为其区间过宽。机制特异得分可以解释 T₁ 为何被检测到，但不能改变这一确认性结论。</p><div class="notice">上述数值仅用于教学，不是实验结果。其目的在于在冻结代码与实验前，使从逐查询得分到最终标签的每一步都可以检查和复现。</div>` },
    ...
    "paper-benchmark": {
      ...
      sections: [
        ...
        { title: "评价指标、等效性与功效", body: `<p><strong>主要估计量：</strong>replica 级最不利控制组合 <em>θ</em><sub>i</sub> 及同时置信区间。<strong>正向证据：</strong>固定 FPR 下的 TPR，以及预注册候选检验族的 family-wise error。<strong>负向证据：</strong>在 <em>δ*</em> 下等效性检验通过率、已知正例上的错误等效率和达到的功效 <em>1−β</em>。<strong>不确定性：</strong>结论不充分率，并拆分为功效不足、控制不稳定、主要通道缺失、端点漂移和机制通道冲突。<strong>证据支持集合：</strong>错误纳入率、集合大小和受控谱系召回率。<strong>辅助任务：</strong>闭集 top-k 与机制特异检验，放入独立的次要 FDR 检验族。<strong>效率：</strong>达到固定错误、等效性和功效目标所需的发现与推断查询数。</p><p>确认性检验族为每个候选一个固定组合的交并检验，并使用 Holm 或预注册 max-statistic 程序跨候选校正。机制特异和控制层诊断属于次要分析，不能升级主要结论。报告对 <em>δ*</em> 的敏感性、层级查询簇区间和逐控制层表现。</p>` },
        ...
        { title: "带成本的实验设计矩阵", body: `<p>以下数值用于给出最低可执行规划，并非不可调整的最终模型选择。教师与学生标识必须在训练前写入冻结清单。由于商业 API 单价会变化，成本首先用训练运行数、API 调用数和 token 等价量报告，随后再代入实际供应商价格。</p><table class="matrix"><thead><tr><th>阶段</th><th>教师 / 学生设计</th><th>训练工作量</th><th>查询分配</th><th>示例审计成本</th></tr></thead><tbody><tr><td>P0 信号试验</td><td>2 个教师家族；1 个学生基础家族；响应/推理/偏好；0.25× 与 1.0× 监督剂量；3 个 replica</td><td>36 个正向学生 + 9 个同基础/共享数据/共同第三教师控制 = <strong>45 次训练</strong></td><td>90 个发现查询 + 180 个推断查询；2 次生成；4 模型盲化面板</td><td>约 26,460 次生成；按每次 600 token 约 15.9M token 等价量</td></tr><tr><td>P1 受控确认性实验</td><td>2 个教师家族；2 个学生基础；3 种机制；2 个剂量；4 个 replica</td><td>96 个正向学生 + 32 个识别控制 = <strong>128 次训练</strong></td><td>150 个发现查询 + 300 个推断查询；2 次生成；5 模型面板</td><td>约 119,700 次生成；约 71.8M token 等价量</td></tr><tr><td>P4 模拟 API 压力测试</td><td>从不同效应和控制中选择 24 个冻结待审计模型；3 种包装/解码设置</td><td>除已有检查点外不新增训练</td><td>300 个留出查询；2 次生成；5 模型面板</td><td>约 52,200 次调用；约 31.3M token 等价量</td></tr><tr><td>P5 商业 API 试验</td><td>3 个待审计端点；5 个候选/控制端点；3 个审计日期</td><td>不训练；仅观察性审计</td><td>120 个发现查询 + 240 个推断查询；2 次生成</td><td><strong>17,280 次调用</strong>；约 10.4M token 等价量</td></tr><tr><td>可选商业确认实验</td><td>5 个待审计端点；8 个候选/控制端点；3 个日期</td><td>不训练；仅在试验达到继续标准后运行</td><td>150 个发现查询 + 300 个推断查询；3 次生成</td><td><strong>52,650 次调用</strong>；约 31.6M token 等价量</td></tr></tbody></table><div class="equation">API\ 成本 = ∑<sub>p</sub>(N<sub>in,p</sub>·rate<sub>in,p</sub> + N<sub>out,p</sub>·rate<sub>out,p</sub>). <span class="eq-label">(8)</span></div><p>训练清单还必须记录实际 GPU 小时、能耗/运行时长、教师生成 token 和存储量。正式主实验应由试验阶段的方差估计触发：只有为了满足冻结的功效目标时，才增加 replica、推断查询或重复生成数。</p>` },
    ...
    "paper-roadmap": {
      ...
      sections: [
        ...
        { title: "分阶段执行计划", body: `<table class="matrix"><thead><tr><th>阶段</th><th>交付物</th><th>决策标准</th></tr></thead><tbody><tr><td>P0 · 信号审计</td><td>在已知单教师学生上测试分歧、罕见错误、偏好和推理信号</td><td>至少一个通道超过共享基础与能力匹配控制</td></tr><tr><td>P0.5 · 可识别性与功效设计</td><td>冻结 replica 级估计量、逐查询得分、查询/机制权重、δ*、α、β*、负控制层、嵌套方差估计器、确认性检验族、盲化面板、污染筛查和发现/推断预算</td><td>所有候选具有可执行功效方案、固定统计量和可识别控制前，不得开启正式测试划分</td></tr><tr><td>P1 · 证据检验核心</td><td>留出自适应探针 + 匹配残差检验 + 三态结论</td><td>跨家族和机制控制误报并保持足够功效</td></tr><tr><td>P2 · 困难负样本</td><td>共享数据、第三教师、兄弟检查点、其他派生关系</td><td>若信号退化为家族相似性，则否定主张</td></tr><tr><td>P3 · 受控辅助任务</td><td>唯一候选识别、角色不同的有限双教师学生</td><td>只在覆盖率和错误纳入可靠时保留</td></tr><tr><td>P4 · API 模拟</td><td>解码、包装、随机性和发现/检验预算</td><td>核心方法必须保持仅输出</td></tr><tr><td>P5 · 商业案例</td><td>披露案例与观察性未知案例</td><td>报告候选条件证据结论，不声称隐藏真实关系</td></tr></tbody></table>` },
        ...
        { title: "可直接用于投稿的汇报模板", body: `<h3>主要候选证据表</h3><table class="matrix"><thead><tr><th>候选</th><th>通过的控制层</th><th>θ̂<sub>i</sub></th><th>同时置信区间</th><th>校正正向 p</th><th>等效性结果</th><th>实际功效</th><th>结论</th><th>查询数</th></tr></thead><tbody><tr><td>Tᵢ</td><td>同基础 / 替代教师 / 共享数据 / 共同第三教师</td><td>效应量</td><td>[L,U]</td><td>p<sub>adj</sub></td><td>位于/不位于 ±δ*</td><td>1−β</td><td>检测到 / 未检测到 / 结论不充分</td><td>发现 + 推断</td></tr></tbody></table><h3>必须配套的表格</h3><ul><li><strong>逐控制层效应：</strong>候选 × 控制层逐行报告，容易负样本与决定可识别性的困难控制不得合并。</li><li><strong>鲁棒性：</strong>端点版本/日期、解码器、系统包装、重复采样、后训练变换、效应变化和结论稳定性。</li><li><strong>失败案例：</strong>候选、失败类别、功效不足/零分布不稳定/通道缺失原因，以及是否降级为结论不充分。</li><li><strong>商业案例：</strong>预注册候选、观察效应、稳定时间窗、成本和明确结论边界。</li></ul><h3>有边界的结果表述</h3><div class="claim-box"><b>检测到</b>“在预注册候选面板、匹配控制、留出协议和多重比较校正下，检测到候选 Tᵢ 的候选条件证据（θ̂=…，同时 CI=…，p<sub>adj</sub>=…）。该结论不是因果教师声明。”</div><div class="claim-box"><b>未检测到</b>“同时置信区间完整位于预注册可忽略效应区域 ±δ* 内，达到的功效为 …；在该分辨率下未检测到候选 Tᵢ 的候选条件证据。”</div><div class="claim-box"><b>结论不充分</b>“由于 …，正向检测与等效性均未建立；当前证据不支持正向或负向谱系结论。”</div><div class="claim-box"><b>商业 API</b>“该结果属于端点级观察性结论，并受版本时间窗限制；未列出的教师、共享数据、后端路由和提供方后训练仍无法排除。”</div><p>局限性部分必须明确讨论候选池不完整、控制不匹配、训练数据重叠、端点漂移、隐藏路由、机制组合设定错误、查询成本，以及行为证据本身无法证明因果关系。</p>` },
    ...
    home: {
      ...
      stats: [["65","篇论文、工具、官方披露和行业报告"],["4","个从问题定义到执行计划的论文工作区"],["5","级区分官方披露、实验和报道的证据体系"]],
      sections: [
