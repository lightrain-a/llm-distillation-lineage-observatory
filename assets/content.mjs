...
export const SITE_CONTENT = {
  ...
  zhPages: {
    ...
    "research-agenda": { eyebrow: "资源 · 长期研究方向", title: "长期研究议程", lead: "围绕蒸馏机制取证、教师影响区间、低查询审计与不确定多阶段谱系恢复形成分层研究路线。", callout: "这些内容是经过提案 agent、敌对审稿 agent 和最终仲裁 agent 辩论后的研究假设，不是已经验证的实验结论。排序综合考虑新颖性、可识别性、开源模型可执行性、统计有效性和实验成本。", sections: [
      ...
      { title: "候选 idea 的 agent 辩论与筛选标准", body: `<p>讨论采用三个相互独立的角色：提案者提出窄方向，敌对审稿人专门攻击新颖性、可识别性、数据与功效，最终仲裁者只保留能够改变<strong>推断目标</strong>而不是仅替换相似度指标的方向。</p><table class="matrix"><thead><tr><th>筛选标准</th><th>必须具备的证据</th><th>自动降级条件</th></tr></thead><tbody><tr><td>新颖性边界</td><td>明确超过教师追踪、通用派生关系检测、参考检查点检测、推理来源分析、集合值来源和 Agent 行为相似度。</td><td>仅提出新的 embedding、prompt 优化器或相似度分数。</td></tr><tr><td>可识别性</td><td>说明哪个潜在事实可恢复，哪些替代生成过程在观测上不可区分。</td><td>没有匹配控制，却把行为相似解释成直接训练边。</td></tr><tr><td>最小因果设计</td><td>固定教师、学生初始化、prompt 池、有效 token、训练预算和后训练，只改变目标因素。</td><td>直接比较训练历史未知的不同发布模型。</td></tr><tr><td>统计有效性</td><td>以独立训练模型为推断单位，支持拒绝判断，并预先冻结多重比较、停止和覆盖率目标。</td><td>使用 prompt 级 p 值，或在最终测试集上调阈值。</td></tr><tr><td>论文级贡献</td><td>形成新的估计目标、校准判定规则、机制结论或可识别性理论。</td><td>只有鲁棒性表格或没有更尖锐主张的 benchmark。</td></tr></tbody></table>` },
      { title: "第一梯队：优先形成论文的三个方向", body: `<table class="matrix comparison-table"><thead><tr><th>排名</th><th>方向</th><th>可发表主张</th><th>最小实验</th><th>致命风险</th></tr></thead><tbody><tr><td>1</td><td><strong>同教师、不同蒸馏机制的黑盒归因</strong></td><td>固定教师后，区分响应 SFT、偏好/评判蒸馏、推理轨迹蒸馏和普通非教师微调，并在未见学生架构上输出校准拒绝判断。</td><td>3 个开源教师 × 2 个学生家族；固定初始化、prompt 内容、有效 token 与计算预算，只改变监督机制。</td><td>检测器可能只学到长度、格式或能力差异，而不是监督机制。</td></tr><tr><td>2</td><td><strong>教师混合比例或行为影响区间</strong></td><td>输出协议内的影响区间或暴露等级，而不是把黑盒行为错误解释成现实训练数据的精确百分比。</td><td>双教师混合比例设为 0、10%、25%、50%、75%、90%、100%，固定总 token；在留出学生上评价区间覆盖率与宽度。</td><td>训练非线性使数据比例与行为影响不等价，必须采用部分识别表述。</td></tr><tr><td>3</td><td><strong>可随时停止的低查询蒸馏审计</strong></td><td>在保持模型级误归因控制的前提下降低平均查询量，并对困难样本自动继续查询、输出明确结论不充分。</td><td>比较固定预算与冻结 prompt 组上的序贯证据更新；报告平均/尾部查询数、灵敏度、误归因率和拒绝率。</td><td>自适应选择下一批探针可能破坏置换、conformal 或交换性保证。</td></tr></tbody></table><div class="claim-box"><b>推荐优先级</b>同教师机制归因具有最干净的因果对照，也最能避开现有“教师是谁”工作的正面竞争；序贯审计最适合先做统计 demo；教师影响区间的 uncertainty-quantification 故事最鲜明。</div>` },
      { title: "第二梯队：必须先通过可行性门槛", body: `<table class="matrix"><thead><tr><th>方向</th><th>达到论文级贡献的条件</th><th>先验可行性门槛</th></tr></thead><tbody><tr><td><strong>来源证据生存曲线</strong></td><td>建立继续 SFT、领域适配、重写、剪枝、量化或合并下不同签名的可预测衰减与半衰期，而不是只给鲁棒性表格。</td><td>小剂量测量能够预测后续退化，并证明不同签名具有稳定不同的衰减规律。</td></tr><tr><td><strong>角色条件多教师归因</strong></td><td>恢复教师分别担任生成器、批评者、评判器、安全过滤器或最终改写器的角色，同时把角色与教师身份分开。</td><td>固定教师身份并交换角色；若方法只能检测最终 rewriter，则方向失败。</td></tr><tr><td><strong>重复蒸馏的有序谱系恢复</strong></td><td>在未知深度和缺失中间模型下恢复 partial order 或 ancestry set，并同时给出不可识别性结果。</td><td>必须超过两两归因分数的简单串联，并区分 A→B→C 与 {A,B}→C；否则应报告不可能性而不是完整图。</td></tr></tbody></table>` },
      { title: "只适合作为 benchmark 或观察性研究的方向", body: `<div class="claim-box"><b>困难负样本与混杂因素基准</b>共享基础模型、共享数据、共同第三教师、能力匹配、打乱教师输出、候选缺失和后训练构成非常有价值的开源资产，但更适合服务于更强的方法或统计论文，不宜单独作为主线。</div><div class="claim-box"><b>受版本限制的商业 API 来源观察</b>endpoint 静默升级、provider routing 和历史 snapshot 缺失使因果真值不可获得。商业端点适合作为版本冻结的观察性案例，不适合作为因果来源方法的主要验证。</div><p>进一步提出“更好的相似度分数”或无约束开放集教师列表不再优先，因为它们与现有教师追踪、派生检测、集合值来源和模型指纹工作重叠过大。</p>` },
      { title: "作为后续框架的 DistillGraph", body: `<div class="steps"><div class="step"><strong>从校准候选检验开始。</strong>把第一篇论文的证据检测结论作为边筛选信号，而不是已证明的边。</div><div class="step"><strong>加入机制与角色条件证据。</strong>只在具有受控真实关系时加入响应、推理、偏好、生成、评判、批评、安全和工具通道。</div><div class="step"><strong>按可用访问条件升级。</strong>把 logit、参考检查点、表示、路由和权重作为可选佐证，而不是核心要求。</div><div class="step"><strong>输出不确定依赖图。</strong>保留未知来源节点、替代边、partial order 和明确拒绝判断。</div></div>` },
      { title: "未来基准扩展", body: "单教师证据检测稳定后，再扩展到机制匹配学生、教师混合剂量网格、角色交换双教师流程、重复蒸馏、缺失中间节点、信号洗白和更大的不完整候选池。" },
