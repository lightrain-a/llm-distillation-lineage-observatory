export const SITE_CONTENT = {
  ...
  sectionRefs: {
    preliminary: [[40,41,54],[40,36,38],[36,37,40],[28,29,30,33],[2,8,9,10,12],[2,3,49,53]],
  ...
  zhPages: {
    home: {
      ...
      stats: [["59","篇论文、工具、官方披露和行业报告"],["7","个从基础概念到评价的研究阶段"],["5","级区分官方披露、实验和报道的证据体系"]],
      sections: [
    ...
    preliminary: {
      eyebrow: "基础概念", title: "背景与定义", lead: "区分知识蒸馏、微调、模型抽取、模型压缩和一般谱系关系所需的核心概念。",
      sections: [
        { title: "什么是大语言模型蒸馏？", body: "大语言模型蒸馏通过训练目标或教师生成数据流程，把教师模型的选定行为或内部知识转移到学生模型。被转移的对象可以是概率分布、隐藏表示、最终响应、推理轨迹、偏好判断、批评反馈或工具交互轨迹。" },
        { title: "教师、学生与蒸馏信号", body: "教师模型负责产生监督，学生模型根据监督更新参数。蒸馏信号才是实际训练目标。同一个教师既可以在白盒流程中提供 logit，也可以通过 API 提供响应，或作为评判器提供排序。" },
        { title: "蒸馏与微调", body: "微调描述在下游目标上更新参数；蒸馏由监督来源定义。学生可以在教师输出上进行微调，但在人工数据或公开数据上的普通微调并不会自动形成教师—学生蒸馏关系。" },
        { title: "蒸馏与模型抽取", body: "模型抽取从攻击者或审计者视角描述通过查询恢复功能或行为。响应蒸馏是训练替代模型的一种机制，因此抽取流程可能包含蒸馏，但两者不能互换。" },
        { title: "蒸馏与模型谱系", body: "模型谱系比蒸馏更广，包含检查点继承、继续预训练、监督微调、模型合并、剪枝、量化和蒸馏。可靠审计必须区分这些边类型，而不是把所有行为相似性都解释为蒸馏。" },
        { title: "为什么需要蒸馏审计？", body: "工业问题包括 API 滥用、未授权能力转移、许可证合规、供应链透明度、安全策略继承和错误归因。实际系统需要带拒绝选项的校准证据，而不是强制指控。" },
      ],
    },
    taxonomy: {
