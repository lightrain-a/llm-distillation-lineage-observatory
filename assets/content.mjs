...
export const SITE_CONTENT = {
  ...
  pageDetails: {
    ...
    "research-agenda": {
      ...
      zh: { overview: "当前议程优先推进三个近期论文方向：同教师机制归因、校准教师影响区间和可随时停止的低查询审计；来源证据生存曲线、角色条件多教师和重复蒸馏有序谱系属于通过可行性门槛后的扩展。DistillGraph 仍是更长期的整合层。", terms: ["蒸馏机制归因", "教师影响区间", "序贯有效审计", "来源证据生存曲线", "角色条件教师", "部分有序谱系"], questions: ["监督机制能否与输出格式和能力提升分离？", "哪些教师影响量只能被部分识别？", "序贯审计何时仍能控制错误率？", "哪些多阶段谱系在根本上不可区分？"] },
    },
  },
  zhPages: {
    "paper-problem": {
    ...
    "research-agenda": { eyebrow: "资源 · 长期研究方向", title: "长期研究议程", lead: "围绕蒸馏机制取证、教师影响区间、低查询审计与不确定多阶段谱系恢复形成分层研究路线。", callout: "这些内容是经过提案 agent、敌对审稿 agent 和最终仲裁 agent 辩论后的研究假设，不是已经验证的实验结论。排序综合考虑新颖性、可识别性、开源模型可执行性、统计有效性和实验成本。", sections: [
      ...
      { title: "未来基准扩展", body: "单教师证据检测稳定后，再扩展到机制匹配学生、教师混合剂量网格、角色交换双教师流程、重复蒸馏、缺失中间节点、信号洗白和更大的不完整候选池 [[16,44,45,59,88]]。" },
