import { NEXT_PAPER_PAGE_DETAILS, NEXT_PAPER_SECTION_REFS, NEXT_PAPER_ZH } from "./next-paper-zh.mjs?v=20260723-concrete-experiment-terms-v1";
...
export const SITE_CONTENT = {
  ...
  sectionRefs: {
    ...
    "research-agenda": [[1,3,6,7,9],[1,6,7],[1,2,3,5,6,7,8,9,16,88,89],[1,3,5,6,7,38,39,44,79,88,90],[1,7,8,9,16,24,88,89],[2,7,9,16,33,49,59,70,71,72,73,74,75,85],[1,4,6,7,9],[16,44,45,76,77,78,79],[44,47,57,58]],
  ...
  pageDetails: {
    ...
    "paper-benchmark": {
      ...
      zh: { overview: "基准区分四类证据：72次平衡受控因果实验、公开 DeepSeek-R1 蒸馏家族、Llama 3.2 与 Minitron 的多教师/剪枝混合流程，以及受版本限制的 OpenRouter 观察研究。只有明确标注教师输出来源的受控训练组用于评价因果敏感性和机制恢复。", terms: ["四种明确训练条件", "可蒸馏教师", "公开监督控制", "打乱监督控制", "公开蒸馏模型对", "OpenRouter 观察"], questions: ["方法能否恢复随机分配的教师与机制？", "公开混合流程上的证据如何变化？", "哪些商业端点结果必须保持结论不充分？"] },
    },
    "paper-roadmap": {
    ...
    "research-agenda": {
      en: { overview: "The agenda now prioritizes three near-term paper directions—same-teacher mechanism attribution, calibrated teacher-influence intervals, and anytime-valid low-query auditing—before conditional work on provenance survival, role-conditioned teachers, and ordered repeated-distillation ancestry. DistillGraph remains the longer-term integration layer.", terms: ["mechanism attribution", "teacher-influence interval", "anytime-valid audit", "provenance survival curve", "role-conditioned teacher", "partial-order lineage"], questions: ["Can supervision mechanism be separated from output format and utility?", "Which teacher-influence quantities are partially identifiable?", "When does sequential auditing preserve valid error control?", "Which multi-stage lineages are fundamentally indistinguishable?"] },
      zh: { overview: "当前议程优先推进三个近期论文方向：同教师机制归因、校准教师影响区间和可随时停止的低查询审计；来源证据生存曲线、角色条件多教师和重复蒸馏有序谱系属于只有完成可行性验证后才考虑的扩展。DistillGraph 仍是更长期的整合层。", terms: ["蒸馏机制归因", "教师影响区间", "序贯有效审计", "来源证据生存曲线", "角色条件教师", "部分有序谱系"], questions: ["监督机制能否与输出格式和能力提升分离？", "哪些教师影响量只能被部分识别？", "序贯审计何时仍能控制错误率？", "哪些多阶段谱系在根本上不可区分？"] },
    },
  },
  zhPages: {
    "paper-problem": {
    ...
    "paper-benchmark": {
      ...
      sections: [
        ...
        { title: "最低规模受控实验", body: `<p>首先只运行一个学生基础模型；只有方法控制误报并达到预注册功效后才扩展。每个“监督机制 × 目标token数量”组合都包含4种数量相同的训练条件。</p><table class="matrix"><thead><tr><th>训练条件</th><th>精确设计</th><th>模型数</th></tr></thead><tbody><tr><td>教师 T₁ 正例</td><td>3种相互隔离的监督机制 × 2种目标token数量 × 3个独立随机种子</td><td><strong>18</strong></td></tr><tr><td>教师 T₂ 正例</td><td>与 T₁ 完全相同的6种“监督机制 × 目标token数量”组合和随机种子编号</td><td><strong>18</strong></td></tr><tr><td>公开监督控制</td><td>3种监督机制 × 2种目标token数量 × 3个随机种子；使用相同提示和token预算，目标来自非教师公开金标或确定性公共规则</td><td><strong>18</strong></td></tr><tr><td>打乱监督控制</td><td>3种监督机制 × 2种目标token数量 × 3个随机种子；在任务家族内打乱合并后的教师目标集合，保持长度和教师输出暴露，但破坏输入—目标对应</td><td><strong>18</strong></td></tr><tr><td>计划训练总数</td><td>全部使用 <code>Qwen/Qwen3-8B-Base</code> 与 LoRA-r16</td><td><strong>72</strong></td></tr></tbody></table><p>未更新的基础检查点保留为描述性 zero-update 基线，但不作为独立训练模型。两种训练数据量分别为<strong>3M和12M教师监督token</strong>。在每个区组内，4种训练条件标签在每个随机种子组内随机分配，并在分析代码冻结前保持盲化。各训练组使用相同提示ID、优化器、更新数、打包 token 预算和评价划分。响应学生只优化最终短答案；推理学生优化结构化中间步骤，并在机制对比中排除最终答案贡献；偏好学生使用固定回答对与 A/B/平局判断，DPO β=0.1。另一候选教师输出训练组用于替代教师比较。</p><p><strong>扩大实验的条件。</strong>达到预注册继续条件后，在 <code>meta-llama/Llama-3.1-8B</code> 上复现每模型12M目标token的4种训练条件：18 个正例和 18 个匹配控制；随后把 <code>deepseek/deepseek-r1-0528</code> 作为未见推理教师运行三个种子。完整 DeepSeek、Llama 3.2 和 Minitron 家族只在上述最低规模受控实验达到预注册标准后评价。</p>` },
        { title: "重复实验层级与推断单位", body: `<p>每个受控条件必须包含多个独立初始化、独立训练的学生模型。提示词、语义模板、重复 API 生成和解码样本都属于嵌套测量，不能当作独立实验单位。主要分析应跨独立训练学生模型估计候选效应，并使用层级 bootstrap 或混合效应模型，把查询簇和重复采样嵌套在每个学生模型内。</p><p>正式评价前固定：独立训练学生模型数量、每个机制的留出查询簇数、每个查询的重复生成数、解码参数、每个控制层的模型数和单元内平均规则。商业端点没有同一训练条件下的多个独立训练模型；跨日期与API会话的重复只评价观察稳定性，不能替代基于独立训练模型的受控推断。</p>` },
        ...
        { title: "评价指标、等效性与功效", body: `<p><strong>主要估计量：</strong>通道特异成对转移效应 <em>θ</em><sub>i,r</sub> 与联合校准的 omnibus <em>Z</em><sub>i,max</sub>。<strong>正向证据：</strong>在 maxT/step-down 校正下、固定 family-wise error 的候选级 TPR。<strong>负向证据：</strong>每个主要通道在预注册 <em>δ</em><sub>r</sub> 下的同时等效性、已知正例上的错误等效率，以及每个通道达到的功效。<strong>机制恢复：</strong>仅在 omnibus 检测成立的候选上报告条件式通道 precision/recall 与置信区间。<strong>不确定性：</strong>结论不充分率，并拆分为功效不足、探针不稳定、接口缺失、端点漂移和通道冲突。<strong>效率：</strong>候选发现调用与留出成对推断调用的数量。</p><p>置换分组保持任务家族、成对编辑、独立训练模型与控制层结构不变。同一个联合零分布同时校准通道选择和候选多重性。结果必须逐通道报告效应，按 G0-G4 来源层分别报告校准，并评价候选池缺失真实教师的情况。</p>` },
    ...
    "paper-roadmap": {
      eyebrow: "下一篇论文 · 主张与执行", title: "推荐论文主张与执行方案", lead: "以成对局部边界转移、选择有效的 maxT 推断、平衡受控蒸馏实验和商业 API 观察案例为核心的候选条件黑盒论文。", callout: "不能把行为证据包装成因果教师归因。第一篇论文先证明逐候选证据检验在困难负样本下具有校准误报率、足够功效和明确的结论不充分输出。",
      sections: [
    ...
    "research-agenda": { eyebrow: "资源 · 长期研究方向", title: "长期研究议程", lead: "围绕蒸馏机制取证、教师影响区间、低查询审计与不确定多阶段谱系恢复形成分层研究路线。", callout: "这些内容是经过提案 agent、敌对审稿 agent 和最终仲裁 agent 辩论后的研究假设，不是已经验证的实验结论。排序综合考虑新颖性、可识别性、开源模型可执行性、统计有效性和实验成本。", sections: [
      { title: "第一篇论文之后仍未解决的问题", body: "第一篇论文聚焦候选条件黑盒蒸馏证据检测。现有教师追踪、开放权重检测、参考检查点和集合值来源工作已经覆盖单候选或候选集合证据 [[1,3,6,7]]。后续问题包括因果教师识别、更大教师集合、角色分离、相对影响、重复蒸馏、缺失中间模型和多阶段信号洗白。" },
      { title: "未来结构化问题", body: "给定待审计模型、候选与未知教师、可选参考检查点和异质访问预算，恢复带类型且具有不确定性的依赖结构。现有集合值来源和依赖恢复工作表明，该任务必须保留未知来源、替代边与不确定结构 [[7,9,59]]。它严格难于候选归因，不能由同一组证据直接支持。" },
      { title: "候选 idea 的 agent 辩论与筛选标准", body: `<p>讨论采用三个相互独立的角色：提案者提出窄方向，敌对审稿人专门攻击新颖性、可识别性、数据与功效，最终仲裁者只保留能够改变<strong>推断目标</strong>而不是仅替换相似度指标的方向 [[1,2,3,5,6,7,8,9,16,88,89]]。</p><table class="matrix"><thead><tr><th>筛选标准</th><th>必须具备的证据</th><th>自动降级条件</th></tr></thead><tbody><tr><td>新颖性边界</td><td>明确超过教师追踪、通用派生关系检测、参考检查点检测、推理来源分析、集合值来源和 Agent 行为相似度。</td><td>仅提出新的 embedding、prompt 优化器或相似度分数。</td></tr><tr><td>可识别性</td><td>说明哪个潜在事实可恢复，哪些替代生成过程在观测上不可区分。</td><td>没有匹配控制，却把行为相似解释成直接训练边。</td></tr><tr><td>最小因果设计</td><td>固定教师、学生初始化、prompt 池、有效 token、训练预算和后训练，只改变目标因素。</td><td>直接比较训练历史未知的不同发布模型。</td></tr><tr><td>统计有效性</td><td>以独立训练模型为推断单位，支持拒绝判断，并预先冻结多重比较、停止和覆盖率目标。</td><td>使用 prompt 级 p 值，或在最终测试集上调阈值。</td></tr><tr><td>论文级贡献</td><td>形成新的估计目标、校准判定规则、机制结论或可识别性理论。</td><td>只有鲁棒性表格或没有更尖锐主张的 benchmark。</td></tr></tbody></table>` },
      { title: "第一梯队：优先形成论文的三个方向", body: `<table class="matrix comparison-table"><thead><tr><th>排名</th><th>方向</th><th>可发表主张</th><th>最小实验</th><th>致命风险</th></tr></thead><tbody><tr><td>1</td><td><strong>同教师、不同蒸馏机制的黑盒归因</strong></td><td>固定教师后，区分响应 SFT、偏好/评判蒸馏、推理轨迹蒸馏和普通非教师微调，并在未见学生架构上输出校准拒绝判断 [[1,3,5,38,39,41,44,47,88,90]]。</td><td>3 个开源教师 × 2 个学生家族；固定初始化、prompt 内容、有效 token 与计算预算，只改变监督机制。</td><td>检测器可能只学到长度、格式或能力差异，而不是监督机制。</td></tr><tr><td>2</td><td><strong>教师混合比例或行为影响区间</strong></td><td>输出协议内的影响区间或暴露等级，而不是把黑盒行为错误解释成现实训练数据的精确百分比 [[7,44]]。</td><td>双教师混合比例设为 0、10%、25%、50%、75%、90%、100%，固定总 token；在留出学生上评价区间覆盖率与宽度。</td><td>训练非线性使数据比例与行为影响不等价，必须采用部分识别表述。</td></tr><tr><td>3</td><td><strong>可随时停止的低查询蒸馏审计</strong></td><td>在保持模型级误归因控制的前提下降低平均查询量，并对困难样本自动继续查询、输出明确结论不充分 [[2,7,16]]。</td><td>比较固定预算与冻结 prompt 组上的序贯证据更新；报告平均/尾部查询数、灵敏度、误归因率和拒绝率。</td><td>自适应选择下一批探针可能破坏置换、conformal 或交换性保证。</td></tr></tbody></table><div class="claim-box"><b>推荐优先级</b>同教师机制归因具有最干净的因果对照，也最能避开现有“教师是谁”工作的正面竞争；序贯审计最适合先做小规模统计实验；教师影响区间的 uncertainty-quantification 故事最鲜明。</div>` },
      { title: "第二梯队：必须先完成可行性验证", body: `<table class="matrix"><thead><tr><th>方向</th><th>达到论文级贡献的条件</th><th>必须先观察到的可行性结果</th></tr></thead><tbody><tr><td><strong>来源证据生存曲线</strong></td><td>建立继续 SFT、领域适配、重写、剪枝、量化或合并下不同签名的可预测衰减与半衰期，而不是只给鲁棒性表格 [[16,24,33,44,89]]。</td><td>较小训练数据量下的测量能够预测后续退化，并证明不同签名具有稳定不同的衰减规律。</td></tr><tr><td><strong>角色条件多教师归因</strong></td><td>恢复教师分别担任生成器、批评者、评判器、安全过滤器或最终改写器的角色，同时把角色与教师身份分开 [[7,56,88]]。</td><td>固定教师身份并交换角色；若方法只能检测最终 rewriter，则方向失败。</td></tr><tr><td><strong>重复蒸馏的有序谱系恢复</strong></td><td>在未知深度和缺失中间模型下恢复 partial order 或 ancestry set，并同时给出不可识别性结果 [[7,9,59]]。</td><td>必须超过两两归因分数的简单串联，并区分 A→B→C 与 {A,B}→C；否则应报告不可能性而不是完整图。</td></tr></tbody></table>` },
      { title: "只适合作为 benchmark 或观察性研究的方向", body: `<div class="claim-box"><b>困难负样本与混杂因素基准</b>共享基础模型、共享数据、共同第三教师、能力匹配、打乱教师输出、候选缺失和后训练构成非常有价值的开源资产，但更适合服务于更强的方法或统计论文，不宜单独作为主线 [[2,7,9,16,59]]。</div><div class="claim-box"><b>受版本限制的商业 API 来源观察</b>endpoint 静默升级、provider routing 和历史 snapshot 缺失使因果真值不可获得。商业端点适合作为版本冻结的观察性案例，不适合作为因果来源方法的主要验证 [[49,59]]。</div><p>进一步提出“更好的相似度分数”或无约束开放集教师列表不再优先，因为它们与现有教师追踪、派生检测、集合值来源和模型指纹工作重叠过大 [[1,2,7,16]]。</p>` },
      { title: "作为后续框架的 DistillGraph", body: `<div class="steps"><div class="step"><strong>从校准候选检验开始。</strong>把第一篇论文的证据检测结论作为边筛选信号，而不是已证明的边 [[1,2,7]]。</div><div class="step"><strong>加入机制与角色条件证据。</strong>只在具有受控真实关系时加入响应、推理、偏好、生成、评判、批评、安全和工具通道 [[5,56,88]]。</div><div class="step"><strong>按可用访问条件升级。</strong>把 logit、参考检查点、表示、路由和权重作为可选佐证，而不是核心要求 [[3,6,8,10,12]]。</div><div class="step"><strong>输出不确定依赖图。</strong>保留未知来源节点、替代边、partial order 和明确拒绝判断 [[7,9,59]]。</div></div>` },
      { title: "未来基准扩展", body: "单教师证据检测稳定后，再扩展到机制匹配学生、教师混合比例与目标token数量网格、角色交换双教师流程、重复蒸馏、缺失中间节点、信号洗白和更大的不完整候选池 [[16,44,45,59,88]]。" },
      { title: "对第一篇论文的依赖", body: "如果仅输出证据检验无法在共享基础模型、共享数据、共同第三教师、API 随机性和候选缺失条件下控制误报与功效，就不应启动 DistillGraph 图重建 [[7,9,16,33,44,45,47,49,59]]。" },
    ]},
  },
  summaryZh: {
    ...
    r69: "DeepSeek 官方披露的 R1 蒸馏学生，可作为推理蒸馏的外部正例，但不能替代多个独立训练模型。",
    ...
    r92: "Qwen3-8B 官方模型卡；在小规模受控行为迁移实验中作为第一个开放教师。",
    r93: "Llama 3.1 8B Instruct 官方模型卡；作为第二个开放教师，并明确允许在许可证约束下使用输出改进其他模型。",
    r94: "Qwen3-1.7B-Base 官方模型卡；作为4种训练条件在相同随机种子下共享的学生初始化。",
    r95: "Qwen3-1.7B 指令模型卡；在小规模受控实验中作为同家族影子教师控制。",
