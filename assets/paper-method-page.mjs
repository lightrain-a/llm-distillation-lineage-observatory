export const PAPER_METHOD_EN = {
  ...
  lead: "A reader-first description of how candidate-specific decisions, structures, and actions may become black-box evidence of teacher-output exposure.",
  callout: "The method is a falsifiable research proposal, not a finished detector. It first asks whether natural behavioral echoes transfer across independently trained students, and only then asks whether a frozen rule can audit one unseen model.",
  sections: [
    {
      title: "Method overview · what we are trying to do",
      body: `<p>We aim to build a candidate-conditioned black-box distillation-evidence test: given an API-only suspect model <em>S</em>, a candidate teacher <em>Tᵢ</em>, and matched controls <em>𝒞ᵢ</em>, the method first discovers behavior that is stable for the candidate but not required for task success, then tests whether <em>S</em> reproduces that behavior on fresh inputs beyond the matched-control level. The motivation is that shared correct answers and surface similarity are easily explained by common model families, public data, capability, prompts, and formatting; the central intuition is that distillation may preserve how a teacher changes decisions, organizes valid responses, or reacts under uncertainty—behavioral echoes that are more diagnostic of training influence than wording alone [[1,2,3,6,7,88,89,90]].</p><div class="definition-box"><b>Behavioral echo</b>A repeatable candidate-specific decision, structural, or action pattern that reappears in a student on fresh tasks even though the pattern is not necessary to obtain a correct answer.</div><div class="survey-note"><b>Not an echo</b>Copying phrases, sharing the same correct answer, or looking generally similar is insufficient when matched controls behave the same way.</div>`
    },
    {
      title: "A concrete example · what counts as evidence",
    ...
    {
      title: "D0 and D1 · first prove transfer, then prove detection",
    ...
    {
      title: "Candidate-only discovery, matched controls, and falsification",
    ...
    {
      title: "Scores, maxT calibration, and the three-state decision",
    ...
    {
      title: "How this differs from ESF and other sensitive fingerprints",
    ...
    {
      title: "Observable echoes are not hidden training mechanisms",
    ...
    {
      title: "Novelty gate, claim boundary, and NO-GO rules",
...
export const PAPER_METHOD_ZH = {
  eyebrow: "下一篇论文 · 方法设计",
  title: "候选特异行为回声检验",
  lead: "从读者直觉出发，说明候选教师特异的决策、结构和行动规律如何成为黑盒蒸馏证据。",
  callout: "这是一套可证伪的研究方案，而不是已经完成的检测器。先验证自然行为回声是否会在独立训练学生总体上传递，再验证冻结规则能否审计一个未见模型。",
  sections: [
    {
      title: "方法概览 · 我们准备做什么",
      body: `<p>我们准备构建一个候选条件黑盒蒸馏证据检验：给定只能通过 API 查询的待审计模型 <em>S</em>、候选教师 <em>Tᵢ</em> 和匹配控制模型 <em>𝒞ᵢ</em>，方法先在候选与控制之间发现教师特异但并非完成任务所必需的行为规律，再在全新输入上检验 <em>S</em> 是否以超过匹配控制的程度稳定复现这些规律。动机是共同正确答案和表面文本相似容易被模型家族、公开数据、能力、提示模板和输出格式解释；核心直觉是蒸馏可能保留教师在输入变化下如何改变决策、如何组织有效回答、以及遇到不确定性时如何行动，这些“行为回声”比措辞相似更接近训练影响 [[1,2,3,6,7,88,89,90]]。</p><div class="definition-box"><b>行为回声</b>学生在全新任务上重复某个候选教师特有、但并非获得正确答案所必需的决策、结构或行动规律。</div><div class="survey-note"><b>不属于行为回声</b>复制短语、共同答对或总体上看起来相似都不够；如果匹配控制也表现相同，就没有候选特异证据。</div>`
    },
    {
      title: "一个贯穿例子 · 什么才算证据",
...
export const PAPER_METHOD_SECTION_REFS = Array.from({ length: 12 }, () => []);
