...
def run_desktop_checks(base_url: str) -> None:
    ...
    try:
        ...
        require("创新性门槛" in method_text and "模型级功效与查询预算" in method_text and "选择有效的候选特异发现" in method_text, "Selection-valid discovery, model-level power planning, or novelty gate is missing")
        require(all(label in method_text for label in ["两个估计目标：群体迁移与单模型检测", "D0 群体可行性", "D1 单模型检测", "训练模型为重采样和置换单位"]), "Population-versus-individual estimands are incomplete")
        require(all(label in method_text for label in ["独立校准后的单模型判定规则", "误指控率", "集合值 precision", "校准范围"]), "Independently calibrated individual decision rule is incomplete")
        ...
        require(all(label in method_text for label in ["D0 群体主张", "D1 检测器主张", "只有结构或行动签名", "停止被动谱系推断"]), "Population/detector boundary or fallback direction is incomplete")
