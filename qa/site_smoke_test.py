...
def run_desktop_checks(base_url: str) -> None:
    ...
    try:
        ...
        require(all(label in method_text for label in ["两阶段验证 · D0 迁移与 D1 检测", "D0 · 群体迁移", "D1 · 单模型检测", "模型级"]), "Population-versus-individual estimands are incomplete")
        require(all(label in method_text for label in ["仅候选发现与反证控制", "打乱候选标签", "联合校准与三态判定", "结论不充分"]), "Discovery, falsification, or calibrated decision rule is incomplete")
        require(all(label in method_text for label in ["与 ESF 及完整性指纹的关系", "已篡改 / 未篡改", "检测到 / 未检测到 / 结论不充分"]), "ESF comparison is incomplete")
        require(all(label in method_text for label in ["创新性、结论范围与停止条件", "方法贡献成立", "主张收缩", "停止条件"]), "Novelty boundary or fallback direction is incomplete")
        ...
        require(all(label in benchmark_text for label in ["推荐实验执行方案 · 公开模型检查、小规模受控实验与正式 D0", "步骤1", "步骤2", "步骤3", "4种训练条件", "单张 A100"]), "Recommended staged demo plan is incomplete")
        ...
        require(all(label in benchmark_text for label in ["32 个训练模型", "8M 目标 token", "随机化与重采样单位", "δpop"]), "D0 population estimand or replication design is incomplete")
        require(all(label in benchmark_text for label in ["满足继续条件", "缩小研究范围后继续", "停止", "全新任务家族"]), "D0 population go/no-go criteria are incomplete")
        require(all(label in benchmark_text for label in ["D0b · 2M与8M目标token比较", "2M 目标 token", "D0c 分解", "不属于零控制"]), "D0 dose-response or decomposition gate is incomplete")
        ...
        require(all(label in benchmark_text for label in ["G0 · D0 达到继续条件后的机制隔离实验", "教师 T₁ 正例", "公开监督控制", "影子教师控制", "72", "DPO β=0.1"]), "Post-D0 mechanism-isolation campaign is incomplete")
        ...
        wait_until(driver, lambda d: d.find_element(By.CSS_SELECTOR, "h1").text == "推荐论文主张与执行方案", "Paper roadmap page did not load in Chinese")
        ...
        require(all(label in roadmap_text for label in ["D0 继续条件", "D1 验证标准", "仅保留结构或行动签名", "停止条件与后续方案"]), "Go/no-go direction rules are incomplete")
