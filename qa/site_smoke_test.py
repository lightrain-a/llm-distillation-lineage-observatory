...
def run_desktop_checks(base_url: str) -> None:
    ...
    try:
        ...
        require(all(label in method_text for label in ["有文献支撑的假设", "行为回声", "合理但暂定", "具有直接 LLM 支持"]), "Evidence-graded method intuition is incomplete")
        require(all(label in method_text for label in ["成对边界转移", "结构响应模式", "错误 / 推理 / 行动模式", "至少一个签名存在"]), "Three behavioral-signature families are incomplete")
        require("创新性门槛" in method_text and "功效感知的查询与训练预算" in method_text and "选择有效的候选特异发现" in method_text, "Selection-valid discovery, power planning, or novelty gate is missing")
        require(all(label in method_text for label in ["签名特异估计量与分层 omnibus", "完整发现—检验算法", "重新运行候选特异特征发现", "maxT", "影子候选库"]), "Selection-valid multi-signature omnibus is incomplete")
        require(all(label in method_text for label in ["分层审计数值示例", "边界", "结构", "错误/行动", "T₃", "结论不充分"]), "Worked multi-signature audit example is incomplete")
        require(all(label in method_text for label in ["Demo 门槛", "32 个独立训练模型", "只有结构或行动签名通过", "停止被动谱系推断"]), "Demo gate or fallback direction is incomplete")
        ...
        require(len(driver.find_elements(By.CSS_SELECTOR, "#dynamic-page .topic-section")) == 16, "Paper benchmark page has the wrong section count")
        require(all(label in benchmark_text for label in ["D0 · 决定性自然签名 demo", "Qwen/Qwen3-8B", "Llama-3.1-8B-Instruct", "Qwen/Qwen3-1.7B-Base", "同家族影子教师", "Qwen/Qwen3-1.7B"]), "Exact decisive-demo model manifest is incomplete")
        require(all(label in benchmark_text for label in ["32 个训练模型", "8M 目标 token", "八个配对种子", "独立单位是训练模型"]), "D0a replication design is incomplete")
        require(all(label in benchmark_text for label in ["至少 13/16", "d≥1.2", "至少提高 0.10", "有条件 GO", "NO-GO", "影子/公开目标臂"]), "D0a go/no-go criteria are incomplete")
        require(all(label in benchmark_text for label in ["D0b 剂量检验", "2M 目标 token", "最大训练量为 64", "D0c 分解实验", "不作为零控制"]), "D0b dose-response or D0c decomposition gate is incomplete")
        ...
        require(all(label in benchmark_text for label in ["重复实验层级与推断单位", "八个配对种子", "商业端点没有训练 replica"]), "Replication hierarchy or inferential-unit specification is incomplete")
        require(all(label in benchmark_text for label in ["带成本的实验设计矩阵", "D0a 高剂量 demo", "D0b 剂量扩展", "G0 机制隔离", "25,920 次调用"]), "Costed staged experiment matrix is incomplete")
        ...
        require(all(label in roadmap_text for label in ["先证伪后扩展", "D0a 失败时停止", "D0 通过后的论文主张"]), "Falsification-first thesis boundary is missing")
        require(all(label in roadmap_text for label in ["D0a · 高剂量自然签名", "D0b · 剂量响应", "G0 · 机制隔离", "G4 · OpenRouter"]), "Staged execution plan is incomplete")
        require(all(label in roadmap_text for label in ["D0 GO", "有条件 GO", "NO-GO", "后备方向"]), "Go/no-go direction rules are incomplete")
        require(all(label in roadmap_text for label in ["可直接用于投稿的汇报模板", "边界", "结构", "错误/行动", "行为回声证据"]), "Publication-ready multi-signature reporting template is incomplete")
